...
describe('Full Workflow E2E', () => {
    ...
    it('should complete workflow: generate → validate', () => {
        ...
        // 1. Generate agent
        const generateOutput = execSync(`node bin/ossa generate chat --name "My Agent" --output ${agentPath}`, {
            encoding: 'utf-8',
            cwd: cwdPath,
        });
    ...
    it('should complete workflow: generate multiple → validate all', () => {
        ...
        for (const agent of agents) {
            ...
            // Generate
            execSync(`node bin/ossa generate ${agent.includes('chat') ? 'chat' : agent.includes('workflow') ? 'workflow' : 'compliance'} --name "${agent}" --output ${agentPath}`, {
                encoding: 'utf-8',
                cwd: cwdPath,
            });
            // Validate
    ...
    it('should handle full development cycle', () => {
        ...
        // Step 1: Generate
        execSync(`node bin/ossa generate workflow --name "Development Agent" --runtime k8s --output ${agentPath}`, {
            encoding: 'utf-8',
            cwd: cwdPath,
        });
        expect(fs.existsSync(agentPath)).toBe(true);
        // Step 2: Validate with verbose
        const validateOutput = execSync(`node bin/ossa validate ${agentPath} --verbose`, {
            encoding: 'utf-8',
            cwd: cwdPath,
        });
