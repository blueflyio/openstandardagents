...
describe('ossa generate command', () => {
  ...
  it('should generate a chat agent', () => {
    ...
    const output = execSync(
      `node bin/ossa generate chat --name "Chat Bot" --output ${outputPath}`,
      {
        encoding: 'utf-8',
  ...
  it('should generate workflow agent with correct capabilities', () => {
    ...
    execSync(
      `node bin/ossa generate workflow --name "Workflow Engine" --output ${outputPath}`,
      {
        encoding: 'utf-8',
        cwd: path.resolve(__dirname, '../../..'),
      }
  ...
  it('should generate compliance agent with low temperature', () => {
    ...
    execSync(
      `node bin/ossa generate compliance --name "Compliance Scanner" --output ${outputPath}`,
      {
        encoding: 'utf-8',
        cwd: path.resolve(__dirname, '../../..'),
      }
  ...
  it('should use custom runtime type', () => {
    ...
    execSync(
      `node bin/ossa generate monitoring --name "K8s Monitor" --runtime k8s --output ${outputPath}`,
      {
        encoding: 'utf-8',
        cwd: path.resolve(__dirname, '../../..'),
      }
  ...
  it('should normalize agent ID', () => {
    ...
    execSync(
      `node bin/ossa generate chat --name "My Agent With SPACES" --output ${outputPath}`,
      {
        encoding: 'utf-8',
        cwd: path.resolve(__dirname, '../../..'),
      }
  ...
  it('should create output directory if it does not exist', () => {
    ...
    execSync(
      `node bin/ossa generate chat --name "Test" --output ${outputPath}`,
      {
        encoding: 'utf-8',
        cwd: path.resolve(__dirname, '../../..'),
      }
