...
`;
            ...
            expect(loaded.ossaVersion).toBe('1.0');
...
`;
        ...
        it('should throw error if file does not exist', async () => {
            ...
            await expect(repository.load(nonExistentPath)).rejects.toThrow('Manifest file not found');
        });
        it('should throw error for unsupported file format', async () => {
            const txtPath = path.join(tempDir, 'test.txt');
            fs.writeFileSync(txtPath, 'some content');
            await expect(repository.load(txtPath)).rejects.toThrow('Unsupported file format');
        });
        it('should throw error for invalid JSON', async () => {
            const jsonPath = path.join(tempDir, 'invalid.json');
            fs.writeFileSync(jsonPath, '{ invalid json }');
            await expect(repository.load(jsonPath)).rejects.toThrow('Failed to parse manifest file');
        });
        it('should throw error for invalid YAML', async () => {
            const yamlPath = path.join(tempDir, 'invalid.yaml');
            fs.writeFileSync(yamlPath, 'invalid: yaml: content: [');
            await expect(repository.load(yamlPath)).rejects.toThrow('Failed to parse manifest file');
        });
    });
    describe('save()', () => {
        ...
        it('should throw error for unsupported save format', async () => {
            ...
            await expect(repository.save(txtPath, manifest)).rejects.toThrow('Unsupported file format');
        });
    });
    describe('exists()', () => {
...
//# sourceMappingURL=manifest.repository.test.js.map