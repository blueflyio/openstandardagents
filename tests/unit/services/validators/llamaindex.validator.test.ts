...
describe('LlamaIndexValidator', () => {
  ...
  describe('validate', () => {
    ...
    it('should warn if index_config is missing', () => {
      ...
      const result = validator.validate(manifest);
      expect(result.valid).toBe(true);
      expect(result.warnings).toContain('Best practice: Configure index_config for LlamaIndex agents');
    });
    ...
    it('should warn if similarity_top_k is missing for query_engine', () => {
      ...
      const result = validator.validate(manifest);
      expect(result.valid).toBe(true);
      expect(result.warnings).toContain('Best practice: Set similarity_top_k for query engine agents');
    });
    ...
    it('should not warn about similarity_top_k for non-query_engine types', () => {
      ...
      const result = validator.validate(manifest);
      expect(result.valid).toBe(true);
      expect(result.warnings).not.toContain('Best practice: Set similarity_top_k for query engine agents');
    });
