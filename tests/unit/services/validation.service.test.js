...
describe('ValidationService', () => {
    ...
    describe('validate()', () => {
        ...
        it('should generate warnings for missing best practices', async () => {
            ...
            const hasDescriptionWarning = result.warnings.some((w) => w.includes('description'));
            expect(hasDescriptionWarning).toBe(true);
            // Check that LLM warning exists
            const hasLLMWarning = result.warnings.some((w) => w.includes('LLM configuration'));
        ...
        it('should handle schema loading errors gracefully', async () => {
            ...
            const result = await validationService.validate(manifest, '999.0');
            expect(result.valid).toBe(false);
            expect(result.errors.length).toBeGreaterThan(0);
        });
    });
