...
class OAASMigrator {
     ...
     */
    async extractAgentInfo(sourceType) {
        switch (sourceType) {
            case 'langchain':
    ...
    async extractCrewAIAgent() {
        ...
        if (fs.existsSync(crewFile)) {
            ...
            if (agentMatch) {
                ...
                if (goalMatch)
                    info.goal = goalMatch[1];
                const backstoryMatch = config.match(/backstory\s*=\s*["']([^"']+)["']/);
                if (backstoryMatch)
                    info.backstory = backstoryMatch[1];
            }
            // Extract tools
    ...
    getFrameworkSpecificConfig(info) {
     ...
     */
    async generateOAASFiles(oaasAgent) {
        ...
        fs.writeFileSync(path.join(agentDir, 'openapi.yaml'), yaml.dump(openApiSpec, { indent: 2 }));
        ...
        if (['production', 'enterprise'].includes(this.options.level)) {
            ...
            fs.writeFileSync(path.join(dataDir, 'examples.json'), JSON.stringify(this.generateExamples(oaasAgent), null, 2));
            fs.writeFileSync(path.join(dataDir, 'training-data.json'), JSON.stringify(this.generateTrainingData(oaasAgent), null, 2));
        }
        // Generate README
...
## Capabilities
${(agent.capabilities || []).map(cap => `- ${typeof cap === 'string' ? cap : cap.name}`).join('\n')}
...
`;
    ...
    async validateMigration() {
        ...
        const agentFile = path.join(this.options.output, '.agents', this.options.source.split('/').pop(), 'agent.yml');
