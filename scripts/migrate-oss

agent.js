  ...
  try {
    ...
    if (agent.metadata) {
      if (agent.metadata.authors) {
        migrated.metadata.annotations.author = Array.isArray(agent.metadata.authors) 
    ...
    // Convert role
    migrated.spec.role = typeof agent.agent.role === 'string' 
      ? agent.agent.role 
      : agent.agent.role;
    ...
    // Convert LLM config
    if (agent.agent.llm) {
      migrated.spec.llm = agent.agent.llm;
    }
    ...
    if (agent.agent.capabilities && Array.isArray(agent.agent.capabilities)) {
      migrated.spec.tools = agent.agent.capabilities.map(cap => {
        ...
        if (cap.description) {
          // Can't add description directly - schema doesn't support it
          // Would need to be in extensions or comments
        }
    ...
    if (agent.agent.observability || agent.agent.monitoring) {
      migrated.spec.observability = agent.agent.observability || agent.agent.monitoring;
    }
    ...
    if (frameworks.includes('mcp') || agent.agent.integration?.mcp) {
      migrated.spec.extensions.mcp = {
        ...
        server_type: agent.agent.integration?.mcp?.protocol || agent.agent.integration?.mcp?.server_type || 'stdio'
      };

      if (agent.agent.capabilities) {
        migrated.spec.extensions.mcp.tools = agent.agent.capabilities.map(cap => ({
    ...
    if (frameworks.includes('kagent') || agent.agent.runtime?.type === 'k8s') {
      ...
      if (agent.agent.runtime?.resources) {
        migrated.spec.extensions.kagent.kubernetes.resourceLimits = agent.agent.runtime.resources;
      }
    }
...
function detectDomain(agent) {
  ...
  if (searchText.includes('infrastructure') || searchText.includes('k8s') || searchText.includes('kubernetes')) {
    return 'infrastructure';
  }
  if (searchText.includes('security') || searchText.includes('compliance')) {
    return 'security';
  }
  if (searchText.includes('data') || searchText.includes('vector') || searchText.includes('database')) {
    return 'data';
  }
...
function detectCapability(agent) {
  ...
  if (searchText.includes('search') || searchText.includes('vector')) return 'search';
...
function detectFrameworks(agent) {
  ...
  for (const [framework, patterns] of Object.entries(MIGRATION_CONFIG.frameworkPatterns)) {
...
console.log('OSSA Agent Migration Tool v1.0 → v0.2.2\n');
...
function findFiles(dir, recursive = false) {
  ...
  function scan(currentDir) {
    ...
    for (const entry of entries) {
      ...
      } else if (entry.name.endsWith('.ossa.yaml') || entry.name.endsWith('.ossa.yml')) {
        files.push(fullPath);
      }
    }
  }
