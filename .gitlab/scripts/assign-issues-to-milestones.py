...
def get_gitlab_url():
    ...
    try:
        ...
        # Extract GitLab URL from remote (e.g., https://gitlab.bluefly.io/llm/openstandardagents.git)
...
def get_project_path():
    ...
    try:
        ...
        remote_url = result.stdout.strip()
        # Extract project path (e.g., llm/openstandardagents)
        if "gitlab" in remote_url:
            parts = remote_url.split("//")[1].split("/")
            if len(parts) >= 3:
...
def main():
    ...
    print("")
    print(f"Review issues: {GITLAB_URL}/{PROJECT_PATH}/-/issues")
