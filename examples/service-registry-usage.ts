...
import createRegistryApiRouter, { SimpleRedisClient } from '../src/api/registry.js';
...
async function basicRegistryExample() {
  ...
  console.log(`✓ Registered: ${registeredService.name} at ${registeredService.endpoint}`);
  ...
  allServices.forEach(service => {
    console.log(`  - ${service.name} v${service.version} (${service.health.status})`);
    console.log(`    Capabilities: ${service.capabilities.map(c => c.name).join(', ')}`);
  });
  ...
  console.log(`Healthy: ${stats.healthyServices}, Unhealthy: ${stats.unhealthyServices}`);
  console.log(`Degraded: ${stats.degradedServices}, Unknown: ${stats.unknownServices}`);
  ...
  console.log(`✓ Updated ${updatedService?.name} to version ${updatedService?.version}`);
...
async function apiServerExample() {
  ...
  const server = app.listen(port, () => {
    ...
    console.log(`  - Registry API: http://localhost:${port}/api/v1/registry/services`);
    console.log(`  - OpenAPI spec available in: src/api/service-registry.openapi.yml`);
  });
...
async function capabilityDiscoveryExample() {
  ...
  aiServices.forEach(s => console.log(`  - ${s.name}: ${s.metadata?.tags?.join(', ')}`));
  ...
  healthyServices.forEach(s => console.log(`  - ${s.name} (${s.capabilities.length} capabilities)`));
