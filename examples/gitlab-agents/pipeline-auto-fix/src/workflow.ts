...
`;
      ...
      context.outputs.create-fix-mr = await this.gitlabClient.createMR(
        context.variables.PROJECT_ID, context.variables.branch.name, context.variables.REF, context.variables.analysis.mr_title, "## 🤖 Auto-Fix for Pipeline #${PIPELINE_ID}\n\n**Status**: Failed ❌ → Fixed ✅\n\n### Root Cause\n${analysis.root_cause}\n\n### Error Type\n`${analysis.error_type}`\n\n### Fix Strategy\n${analysis.fix_strategy}\n\n### Files Changed\n${analysis.affected_files.map(f => `- \\`${f}\\``).join('\\n')}\n\n### Commit\n${commit.short_id}: ${analysis.commit_message}\n\n### Testing\n- [ ] Pipeline passes on this MR\n- [ ] Code review approved\n- [ ] Ready to merge\n\n---\n🤖 Generated by: pipeline-auto-fix agent\n📊 Confidence: ${analysis.confidence}%\n🔗 Failed pipeline: [#${PIPELINE_ID}](${CI_PROJECT_URL}/-/pipelines/${PIPELINE_ID})\n"
      );
