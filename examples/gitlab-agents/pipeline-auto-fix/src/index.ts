...
app.get('/health', (req, res) => {
  res.json({ status: 'healthy', agent: 'pipeline-auto-fix' });
});

// Ready check endpoint
app.get('/ready', (req, res) => {
  res.json({ status: 'ready', agent: 'pipeline-auto-fix' });
});

// Webhook endpoint
app.post('/webhook/pipeline-auto-fix', async (req, res) => {
  ...
  try {
    ...
    if (!matchesFilter(event, {"status":"failed","ref":["main","development","/^release\\/.*/","/^feature\\/.*/"]})) {
