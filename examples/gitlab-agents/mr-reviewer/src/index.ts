...
app.get('/health', (req, res) => {
  res.json({ status: 'healthy', agent: 'mr-reviewer' });
});

// Ready check endpoint
app.get('/ready', (req, res) => {
  res.json({ status: 'ready', agent: 'mr-reviewer' });
});

// Webhook endpoint
app.post('/webhook/mr-reviewer', async (req, res) => {
  ...
  try {
    ...
    if (!matchesFilter(event, {"action":["opened","reopened","updated"]})) {
