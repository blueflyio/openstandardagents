...
export class CapabilityMatcher {
     ...
     */
    async matchCapabilities(agentCapabilities, requiredCapabilities, context) {
        // 1. Direct domain matching
        const domainMatch = this.matchDomains(agentCapabilities.domains, requiredCapabilities.domains);
        // 2. Semantic domain similarity
        const semanticMatch = this.calculateSemanticSimilarity(agentCapabilities.domains, requiredCapabilities.domains);
        // 3. Operation matching
        const operationMatch = this.matchOperations(agentCapabilities.operations || [], requiredCapabilities.operations || []);
        // 4. Specialization compatibility
        const specializationMatch = this.matchSpecializations(agentCapabilities.specializations, requiredCapabilities.specializations);
        // 5. Context-aware scoring adjustments
        const contextAdjustment = this.applyContextualAdjustments({ domainMatch, operationMatch, specializationMatch }, context);
        // Calculate weighted final score
        const baseScore = domainMatch.score * 0.35 +
        ...
        const missingCapabilities = requiredCapabilities.domains.filter(domain => !agentCapabilities.domains.includes(domain));
        const excessCapabilities = agentCapabilities.domains.filter(domain => !requiredCapabilities.domains.includes(domain));
        return {
     ...
     */
    async matchPerformance(agentPerformance, requiredPerformance, slaRequirements) {
        const reasons = [];
        const warnings = [];
        // 1. Throughput compatibility
        const throughputCompatible = this.assessThroughputCompatibility(agentPerformance.throughput, requiredPerformance.throughput);
        ...
        const latencyCompatible = this.assessLatencyCompatibility(agentPerformance.latency, requiredPerformance.latency);
        ...
        const resourceCompatible = this.assessResourceCompatibility(agentPerformance.limits, requiredPerformance.limits);
        ...
        const score = throughputScore * this.performanceWeights.throughput +
     ...
     */
    async rankAgents(candidates, requirements, context) {
        ...
        for (const candidate of candidates) {
            ...
            if (requirements.performance && candidate.manifest.performance) {
                performanceMatch = await this.matchPerformance(candidate.manifest.performance, requirements.performance);
            }
            ...
            const typePreferenceScore = this.calculateTypePreferenceScore(candidate.manifest.agentType, requirements.preferences);
            ...
            const overallScore = capabilityMatch.score * weights.capability +
     ...
     */
    async composeEnsemble(rankedAgents, taskRequirements) {
        const composition = [];
        const reasoning = [];
        ...
        for (const agent of rankedAgents.slice(1, 4)) {
            ...
            if (relevantDomains.length > 0) {
                ...
                relevantDomains.forEach(domain => {
                    ...
                    if (index > -1)
                        uncoveredDomains.splice(index, 1);
                });
            }
        }
        // Add validator if needed for high-stakes tasks
        if (taskRequirements.complexity === 'expert') {
            const validatorCandidate = rankedAgents.find(agent => agent.manifest.agentType === 'critic' ||
    ...
    matchDomains(agentDomains, requiredDomains) {
        const reasons = [];
        const warnings = [];
    ...
    calculateSemanticSimilarity(agentDomains, requiredDomains) {
    ...
    matchOperations(agentOps, requiredOps) {
        const reasons = [];
        const warnings = [];
    ...
    matchSpecializations(agentSpec, requiredSpec) {
        const reasons = [];
        const warnings = [];
        if (!requiredSpec) {
    ...
    applyContextualAdjustments(matches, context) {
        if (!context)
            return 0;
        let adjustment = 0;
        // Urgency adjustments
    ...
    assessThroughputCompatibility(agentThroughput, requiredThroughput) {
    ...
    assessLatencyCompatibility(agentLatency, requiredLatency) {
    ...
    assessResourceCompatibility(agentLimits, requiredLimits) {
        // Simplified resource compatibility check
        return { compatible: true };
    }
    calculateFreshnessScore(manifest) {
    ...
    calculateTypePreferenceScore(agentType, preferences) {
        if (!preferences)
            return 1.0;
        if (preferences.preferredTypes?.includes(agentType))
            return 1.0;
        if (preferences.avoidTypes?.includes(agentType))
            return 0.3;
        return 0.8; // Neutral score
    }
    getPrimaryResponsibilities(manifest, taskRequirements) {
    ...
    calculateEnsembleConfidence(composition, rankedAgents) {
