...
import UADPDiscoveryEngine from './uadp-discovery.js';
import * as fs from 'fs';
import * as path from 'path';
import * as yaml from 'js-yaml';
// Global discovery engine instance
let discoveryEngine;
export function addUADPCommands(program) {
        ...
        try {
            ...
            const agent = {
                ...
                metadata: {
                    ...
                    conformance_tier: agentSpec.metadata?.annotations?.['ossa.io/conformance-level'] || 'advanced',
    ...
    discovery
        ...
        try {
            ...
            if (options.capabilities) {
                discoveryOptions.capabilities = options.capabilities.split(',').map((s) => s.trim());
            }
            if (options.protocols) {
                discoveryOptions.protocols = options.protocols.split(',').map((s) => s.trim());
            }
            if (options.tier) {
                discoveryOptions.conformance_tier = options.tier;
            }
            if (options.frameworks) {
                discoveryOptions.compliance_frameworks = options.frameworks.split(',').map((s) => s.trim());
            }
            if (options.limit) {
...
// Helper functions
async function loadAgentSpec(agentPath) {
    ...
    try {
        ...
        console.log(chalk.red('❌ Invalid agent.yml:'), error.message);
        return null;
    }
}
function extractCapabilities(agentSpec) {
    const capabilities = [];
    if (agentSpec.spec?.capabilities) {
        // Handle different capability formats
        if (Array.isArray(agentSpec.spec.capabilities)) {
            capabilities.push(...agentSpec.spec.capabilities.map((cap) => typeof cap === 'string' ? cap : cap.name));
        }
...
function extractProtocols(agentSpec) {
    const protocols = [];
    if (agentSpec.spec?.protocols) {
        if (Array.isArray(agentSpec.spec.protocols)) {
            protocols.push(...agentSpec.spec.protocols.map((protocol) => ({
            ...
            protocols.push(...agentSpec.spec.protocols.supported.map((name) => ({
                name,
                version: '1.0.0',
...
function extractComplianceFrameworks(agentSpec) {
    const frameworks = [];
    if (agentSpec.spec?.compliance_frameworks) {
        frameworks.push(...agentSpec.spec.compliance_frameworks.map((framework) => typeof framework === 'string' ? framework : framework.name));
    }
    // Extract from annotations
    const complianceAnnotation = agentSpec.metadata?.annotations?.['ossa.io/compliance-frameworks'];
    if (complianceAnnotation) {
        frameworks.push(...complianceAnnotation.split(',').map((s) => s.trim()));
    }
    return frameworks;
}
