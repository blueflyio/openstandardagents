...
import { validator, ValidationFormatter } from '../validation/api-validator.js';
...
// =====================================================================
async function validateAgentFile(options) {
    ...
    const agent = options.file.endsWith('.yaml') || options.file.endsWith('.yml')
...
async function validateAgentById(agentId, options) {
...
async function validateAllAgents(options) {
...
async function validateWorkflowFile(options) {
    if (!await pathExists(options.file)) {
    ...
    const content = readFileSync(options.file, 'utf-8');
    const workflow = options.file.endsWith('.yaml') || options.file.endsWith('.yml')
        ? require('yaml').parse(content)
        : JSON.parse(content);
    const validationOptions = {
        strict: options.strict,
...
async function validateWorkflowById(workflowId, options) {
...
async function validateWorkflowDirectory(options) {
    ...
    for (const file of files) {
        ...
        try {
            ...
            const workflow = file.endsWith('.yaml') || file.endsWith('.yml')
                ? require('yaml').parse(content)
                : JSON.parse(content);
...
async function validateBatch(options) {
...
async function validateRuntime(options) {
    ...
    try {
        ...
        const healthResult = await validator.validateAPIResponse('/health', 'GET', healthResponse.data);
        // Test metrics endpoint
        spinner.text = 'Validating metrics endpoint...';
        const metricsResponse = await ossaClient.getMetrics();
        const metricsResult = await validator.validateAPIResponse('/metrics', 'GET', metricsResponse.data);
        // Test agent listing
        spinner.text = 'Validating agents endpoint...';
        const agentsResponse = await ossaClient.listAgents({ limit: 5 });
        const agentsResult = await validator.validateAPIResponse('/agents', 'GET', agentsResponse.data);
        spinner.stop();
...
async function validateCompliance(options) {
    ...
    try {
        ...
        Object.entries(complianceReport.frameworks).forEach(([framework, data]) => {
...
async function runValidationWizard() {
...
function detectFileType(filePath) {
    ...
    try {
        ...
        if (parsed.openapi || parsed.swagger)
            return 'openapi';
        if (parsed.spec && parsed.spec.conformance_tier)
            return 'agent';
...
async function attemptAgentFixes(agent, result, filePath) {
    ...
    if (capabilityNamingError && fixedAgent.spec.capabilities.primary) {
        fixedAgent.spec.capabilities.primary = fixedAgent.spec.capabilities.primary.map(cap => cap.toLowerCase().replace(/[_\s]+/g, '-'));
...
async function validateOpenAPIWizard(specFile) {
...
async function validateAgentWizard(source) {
    switch (source) {
        case 'file':
...
async function validateRuntimeWizard() {
