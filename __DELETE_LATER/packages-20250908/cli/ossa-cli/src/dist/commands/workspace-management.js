...
export function createWorkspaceManagementCommands() {
        ...
        if (await fs.pathExists(searchPath)) {
            ...
            for (const dir of dirs) {
                ...
                if (await fs.pathExists(workspaceConfig)) {
                    const config = yaml.load(await fs.readFile(workspaceConfig, 'utf-8'));
                    workspaces.push({
...
async function importWorkspace(source, options) {
...
async function exportWorkspace(workspacePath, options) {
...
async function manageWorkspaceConfig(action, key, value) {
...
async function showWorkspaceMetrics(options) {
    ...
    try {
        ...
        if (await fs.pathExists(registryPath)) {
            ...
            metrics.agents.active = Object.values(registry).filter((a) => a.status === 'active').length;
...
async function restoreWorkspace(backup, options) {
...
// Helper functions
async function applyWorkspaceTemplate(workspacePath, template) {
...
async function applyWorkspaceFixes(fixes) {
    for (const fix of fixes) {
        switch (fix.type) {
            ...
            case 'create_registry':
                ...
                break;
            case 'update_config':
                const config = yaml.load(await fs.readFile(fix.path, 'utf-8'));
