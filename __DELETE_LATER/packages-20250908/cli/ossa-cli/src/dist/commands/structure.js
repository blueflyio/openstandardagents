...
export function createStructureCommands() {
    ...
    cmd.command('analyze')
        ...
        const analyses = project
            ? [await analyzer.analyzeProject(project)].filter(a => a !== null)
    ...
    cmd.command('validate')
        ...
        violations.forEach(v => {
            ...
            v.violations.forEach(violation => console.log(`  ${chalk.red('❌')} ${violation}`));
        });
        if (options.fix) {
...
class ProjectStructureAnalyzer {
    ...
    async analyzeAllProjects() {
        ...
        const analyses = await Promise.all(Array.from(projectPaths).map(async (projectPath) => {
    ...
    async analyzeProjectPath(projectPath, name) {
        try {
    ...
    async scanCurrentStructure(projectPath) {
        try {
    ...
    findViolations(current, recommended, type) {
        const violations = [];
        ...
        const rootFiles = current.filter(file => !file.includes('/') &&
    ...
    async findCleanupItems(projectPath) {
        const items = [];
        try {
    ...
    calculateScore(structure, violations, cleanup) {
        let score = 100;
...
class StructureValidator {
    ...
    async createMissingDirectories(analysis) {
        const requiredDirs = [
            'src',
            'tests/unit',
            'tests/integration',
    ...
    async moveMisplacedFiles(analysis) {
        // Move test files to tests/ directory
        const testFiles = analysis.currentStructure.filter(f => /\.(test|spec)\.(js|ts|jsx|tsx)$/.test(f) &&
            !f.startsWith('tests/'));
...
class ProjectCleaner {
    ...
    async cleanAllProjects(options = {}) {
    ...
    async cleanProject(projectName, options = {}) {
        const analyzer = new ProjectStructureAnalyzer(this.workspaceRoot);
        ...
        try {
            ...
            if (options.aggressive) {
                tempPatterns.push('**/node_modules/**', '**/dist/**', '**/.cache/**');
            }
            for (const pattern of tempPatterns) {
                const files = await glob(pattern, {
                    cwd: analysis.path,
...
class ProjectStandardizer {
    ...
    async standardizeProject(projectName, options) {
        const analyzer = new ProjectStructureAnalyzer(this.workspaceRoot);
        const analysis = await analyzer.analyzeProject(projectName);
        if (!analysis)
            return;
...
function displayDetailedAnalysis(analysis) {
    ...
    if (analysis.cleanupItems.length > 0) {
        console.log(chalk.yellow('\nCleanup Items:'));
        analysis.cleanupItems.slice(0, 5).forEach(item => console.log(`  🧹 ${item}`));
