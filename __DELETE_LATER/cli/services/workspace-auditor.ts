...
export class WorkspaceAuditor extends EventEmitter {
  ...
  constructor(workspacePath: string = '/Users/flux423/Sites/LLM/.agents-workspace') {
    super();
   ...
   */
  startAuditing(intervalMs: number = 60000): void {
   ...
   */
  private async performAudit(): Promise<AuditReport> {
   ...
   * Find all .agents directories
   */
  private findAgentDirectories(basePath: string): string[] {
   ...
   */
  private async auditAgentDirectory(agentPath: string): Promise<AgentConfig> {
    const issues: string[] = [];
        ...
        if (!fs.existsSync(filePath)) {
          ...
          try {
            ...
            if (file === 'agent.yaml' && parsed) {
              ...
              if (!parsed.name) issues.push('agent.yaml missing "name" field');
              if (!parsed.version) issues.push('agent.yaml missing "version" field');
              if (!parsed.capabilities) issues.push('agent.yaml missing "capabilities" field');
            }
          } catch (e) {
