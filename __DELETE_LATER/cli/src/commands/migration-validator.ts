...
class MigrationValidator {
   ...
   */
  async validateOSSACompliance(filePath: string): Promise<ValidationResult> {
   ...
   */
  private validateCoreStructure(agent: any, result: ValidationResult): void {
   ...
   */
  private validateMetadata(agent: any, result: ValidationResult): void {
   ...
   * Validate labels
   */
  private validateLabels(labels: any, result: ValidationResult): void {
    if (!labels.tier) {
      result.issues.push({
        type: 'warning',
   ...
   */
  private validateAnnotations(annotations: any, result: ValidationResult): void {
   ...
   */
  private validateSpec(agent: any, result: ValidationResult): void {
   ...
   */
  private validateAPIFirst(agent: any, result: ValidationResult): void {
    const spec = agent.spec;
    if (!spec) return;
   ...
   */
  private validateDiscovery(agent: any, result: ValidationResult): void {
    const spec = agent.spec;
    if (!spec) return;
   ...
   */
  private validateStrictCompliance(agent: any, result: ValidationResult): void {
    const spec = agent.spec;
  ...
  private generateSummary(): any {
    ...
    const errorCount = this.validationResults.reduce(
      (sum, r) => sum + r.issues.filter(i => i.type === 'error').length, 0
    );
    const warningCount = this.validationResults.reduce(
      (sum, r) => sum + r.issues.filter(i => i.type === 'warning').length, 0
    );
...
class RollbackManager {
  private rollbackLog: string = '.ossa-rollback.json';

  /**
   * Create rollback entry
   */
  async createRollbackEntry(originalPath: string, backupPath: string): Promise<void> {
   ...
   * Execute rollback
   */
  async executeRollback(options: RollbackOptions = {}): Promise<void> {
    const rollbackData = await this.loadRollbackData();
   ...
   */
  private async selectiveRollback(entries: RollbackEntry[]): Promise<RollbackEntry[]> {
   ...
   */
  private async performRollback(entries: RollbackEntry[], options: RollbackOptions): Promise<void> {
   ...
   * Load rollback data
   */
  private async loadRollbackData(): Promise<any> {
    try {
 ...
 */
export function createMigrationValidatorCommands(): Command {
