...
async function registerAgent(options) {
    ...
    try {
        ...
        await fs.writeFile(path.join(agentDir, 'agent.yaml'), yaml.dump(manifest));
...
async function listAgents(options) {
    ...
    // Apply filters
    if (filter) {
        agents = agents.filter((agent) => agent.manifest?.spec?.capabilities?.some((cap) => cap.id?.includes(filter)));
    }
    if (status) {
...
async function deployAgent(name, options) {
...
async function startAgent(name, options) {
...
async function getAgentStatus(name, options) {
    if (options.all || !name) {
        ...
        const table = Table.table([
            ...
            ...await Promise.all(Object.entries(registry).map(async ([id, agent]) => {
...
async function validateAgent(manifestPath, options) {
...
function applyFixes(manifest, fixes) {
...
async function updateAgentStatus(name, status) {
    const registryPath = path.join('.agents-workspace', 'agents', 'registry.json');
    if (await fs.pathExists(registryPath)) {
        const registry = await fs.readJson(registryPath);
        if (registry[name]) {
...
async function getAgentRuntimeStatus(name) {
    try {
...
async function updateAgent(name, options) {
    ...
    try {
        ...
        if (version) {
            // Update version
            const agentManifestPath = path.join('.agents', name, 'agent.yaml');
            const manifest = yaml.load(await fs.readFile(agentManifestPath, 'utf-8'));
...
async function removeAgent(name, options) {
...
async function checkAgentHealth(name, options) {
    if (options.all || !name) {
        // Check all agents
        const registryPath = path.join('.agents-workspace', 'agents', 'registry.json');
        const registry = await fs.readJson(registryPath);
...
async function performHealthCheck(name) {
    const checks = [];
    let healthy = true;
...
async function viewAgentLogs(name, options) {
...
async function executeAgentCommand(name, command, options) {
