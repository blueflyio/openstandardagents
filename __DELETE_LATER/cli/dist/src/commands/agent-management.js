...
export function createAgentManagementCommands() {
...
async function checkAgentStatus(agent, options) {
    try {
...
async function validateAgent(agentPath, options) {
    try {
...
async function updateAgent(agent, options) {
    try {
...
async function deleteAgent(agent, options) {
    try {
...
async function cloneAgent(source, target, options) {
    try {
...
async function deployAgent(agent, options) {
    try {
...
function createAgentDirectoryStructure(agentDir) {
    const dirs = [
        'behaviors',
        'config',
        'data',
...
function generateAgentSpec(name, options) {
    return {
        ossa: '0.1.8',
        metadata: {
            name,
...
function generateOpenApiSpec(name, agentSpec) {
    return {
        openapi: '3.1.0',
        info: {
...
function createAgentFiles(agentDir, name, agentSpec) {
    // Create README
...
function generateFrameworkSupport(frameworks) {
    const support = {};
...
async function findAgents(searchPath, filters = {}) {
    const agents = [];
        ...
        for (const file of files) {
            try {
                ...
                if (agent.ossa === '0.1.8') {
                    ...
                    // Apply filters
                    if (filters.tier && agentData.tier !== filters.tier)
                        continue;
                    if (filters.domain && agentData.domain !== filters.domain)
                        continue;
                    if (filters.status && agentData.status !== filters.status)
                        continue;
...
function displayAgentsTable(agents, detailed) {
    ...
    agents.forEach((agent, index) => {
        ...
        if (detailed) {
            if (agent.spec?.protocols) {
                console.log(`   ${chalk.gray('Protocols:')} ${agent.spec.protocols.map((p) => p.name).join(', ')}`);
            }
...
function resolveAgentPath(agent) {
...
function loadAgentSpec(agentPath) {
...
async function displayHealthMetrics(agentPath) {
...
async function displayPerformanceMetrics(agentPath) {
    console.log(chalk.blue('\n  Performance Metrics:'));
...
async function displayComplianceStatus(agentPath) {
    console.log(chalk.blue('\n  Compliance Status:'));
...
async function performAgentValidation(agentPath, options) {
    const agentSpec = loadAgentSpec(agentPath);
    if (!agentSpec) {
...
function displayValidationResult(agentName, validation) {
    if (validation.valid) {
...
async function generateValidationReport(results, reportFile) {
    const report = {
