...
class LegacyFormatConverter {
     ...
     */
    async convertAgent(sourcePath, outputPath) {
     ...
     */
    async parseSource(content, format) {
        switch (format) {
            case 'json':
            case 'ossa':
            case 'ossa-legacy':
     ...
     */
    transformToOSSA(legacy, detection) {
        const ossa = {
            ossa: '0.1.8',
     ...
     */
    transformMetadata(legacy, detection) {
        const metadata = {
     ...
     */
    transformSpec(legacy, detection) {
        const spec = {
            agent: {
                name: this.extractName(legacy),
                description: this.extractDescription(legacy),
     ...
     */
    async writeOSSAAgent(ossaAgent, outputPath) {
 ...
 */
async function batchConvert(pattern, outputDir, options) {
 ...
 */
export function createLegacyConverterCommands() {
