...
function validateAgent(agentPath, options) {
    ...
    if (!fs.existsSync(roadmapMetaFile)) {
        ...
        try {
            ...
            if (!roadmapMeta.roadmap_files || roadmapMeta.roadmap_files.length === 0) {
                ...
                const missingRoadmapFiles = roadmapMeta.roadmap_files.filter((file) => !fs.existsSync(path.join(roadmapDir, file)));
    ...
    try {
        ...
        if (!agent.spec?.protocols || !Array.isArray(agent.spec.protocols)) {
            ...
            const hasOpenAPI = agent.spec.protocols.some((p) => p.name === 'openapi');
            if (!hasOpenAPI) {
                issues.push('❌ Missing OpenAPI protocol (required by OSSA v0.1.8)');
                valid = false;
            }
            const hasUADP = agent.spec.protocols.some((p) => p.name === 'uadp');
            if (!hasUADP) {
        ...
        if (valid && issues.length === 0) {
            ...
            console.log(chalk.gray('   Protocols:'), agent.spec.protocols?.map((p) => p.name).join(', ') || 'none');
                    ...
                    try {
                        ...
                        if (agent.ossa === '0.1.8') {
                            ...
                            agents.push({
                                ...
                                protocols: agent.spec?.protocols?.map((p) => p.name).join(', ') || 'none',
...
function getRoadmapIcon(roadmapStatus) {
...
function createAgentRoadmaps(agentDir, name, currentVersion, domain, tier) {
...
function createRoadmapContent(name, version, domain, tier, versionIndex) {
...
function generateMilestones(name, version, domain, tier, versionIndex) {
    const milestones = [
...
function generateCapabilities(domain, tier, versionIndex) {
...
function generateDependencies(tier, versionIndex) {
...
function generateSuccessCriteria(domain, tier, versionIndex) {
    const criteria = [
