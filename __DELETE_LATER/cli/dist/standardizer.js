...
export class OSSAStandardizer {
    constructor(workspaceRoot = '/Users/flux423/Sites/LLM') {
        this.projects = [];
        this.branchPrefix = 'feature/0.1.8-ossa-standardization';
    ...
    async discoverProjects() {
        ...
        try {
            ...
            this.projects = await Promise.all(agentsDirs.map(async (agentsPath) => {
    ...
    determineProjectType(projectPath) {
        const relativePath = path.relative(this.workspaceRoot, projectPath);
    ...
    async listExistingAgents(agentsPath) {
        try {
    ...
    async standardizeAll() {
        ...
        console.log(boxen(chalk.cyan.bold('🔧 OSSA v0.1.8 Agent Standardization\n') +
            ...
            chalk.gray('• {project}-SPECIALIST.ossa.yml (Advanced OSSA file)'), { padding: 1, borderColor: 'cyan', borderStyle: 'double' }));
    ...
    async standardizeOneByName(projectName) {
    ...
    async getProjectInfoByName(projectName) {
    ...
    createBatches() {
        return {
            'Critical Projects (AI Models + Core NPM)': this.projects.filter(p => p.type === 'ai-model' ||
                ['agent-brain', 'agent-router', 'agent-studio', 'workflow-engine'].includes(p.name)),
            'Integration Projects (Providers + Orchestration)': this.projects.filter(p => p.type === 'drupal-module' &&
                (p.name.includes('provider') || p.name.includes('orchestra') || p.name.includes('workflow'))),
            'Specialized Projects (Domain-specific)': this.projects.filter(p => !['Critical Projects', 'Integration Projects'].some(batch => this.createBatches()[batch]?.some(bp => bp.path === p.path)))
        };
    }
    async processBatch(projects) {
        for (const project of projects) {
            await this.standardizeProject(project);
        }
    }
    async standardizeProject(project) {
    ...
    async createFeatureBranch(project) {
        ...
        try {
            ...
            console.log(chalk.yellow(`  ⚠ Git branching skipped for ${project.name}: ${error.message}`));
        }
    }
    ...
    generateAgentTemplates(project) {
        const baseMetadata = {
    ...
    determineDomain(name, type) {
        ...
        if (name.includes('brain') || name.includes('vector'))
            return 'vector-database';
        return 'general-purpose';
    }
    determinePurpose(name, type) {
        if (name.includes('rfp'))
            return 'rfp-processing';
        if (name.includes('policy') || name.includes('gov'))
    ...
    generateCoreDescription(project) {
    ...
    generateIntegrationDescription(project) {
    ...
    generateTroubleshootDescription(project) {
    ...
    generateIntegrationExpertise(project) {
    ...
    generateTroubleshootExpertise(project) {
    ...
    generateCoreCapabilities(project) {
        ...
        if (project.type === 'drupal-module') {
            baseCapabilities.push({ name: 'entity_operations', description: 'Manage Drupal entities and data structures', project_specific: true }, { name: 'hook_implementation', description: 'Implement and manage Drupal hooks', project_specific: true });
        }
        else if (project.type === 'npm-package') {
            baseCapabilities.push({ name: 'api_management', description: 'Manage package API and public interfaces', project_specific: true }, { name: 'cli_operations', description: 'Handle command-line interface functionality', project_specific: true });
        }
        else if (project.type === 'ai-model') {
            baseCapabilities.push({ name: 'model_inference', description: 'Execute model inference and prediction tasks', project_specific: true }, { name: 'data_processing', description: 'Handle model input/output data processing', project_specific: true });
        }
        return baseCapabilities;
    }
    generateIntegrationCapabilities(project) {
        return [
    ...
    generateTroubleshootCapabilities(project) {
        return [
    ...
    generateIntegrationPoints(project) {
        const integrationMap = {
    ...
    async createAgentStructure(project, templates) {
        ...
        for (const [agentType, config] of Object.entries(templates)) {
            ...
            // Write agent.yml
            await fs.writeFile(path.join(agentDir, 'agent.yml'), yaml.stringify(config, { indent: 2 }));
            // Write README.md
            await fs.writeFile(path.join(agentDir, 'README.md'), this.generateAgentReadme(config, agentType, project));
        }
        // Create advanced OSSA file within per-project generated folder
        await fs.ensureDir(projectAgentsBase);
        await fs.writeFile(path.join(projectAgentsBase, `${project.name}-SPECIALIST.ossa.yml`), this.generateAdvancedOSSAFile(project, templates.core));
    }
    generateAgentReadme(config, type, project) {
...
`;
    ...
    generateAdvancedOSSAFile(project, coreConfig) {
        return `# ==============================================================================
...
`;
 ...
 \nCo-Authored-By: OSSA-CLI <noreply@bluefly.ai>`);
            ...
            console.log(chalk.yellow(`  ⚠ Git commit skipped for ${project.name}: ${error.message}`));
        }
    }
    titleCase(str) {
        return str.split(/[-_]/)
    ...
    async resolveProjectByName(projectName) {
        const candidates = [
