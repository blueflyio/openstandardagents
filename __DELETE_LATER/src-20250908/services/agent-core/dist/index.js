...
// Get agents by type
app.get('/api/v1/agents/type/:type', async (req, res) => {
    try {
...
app.post('/api/v1/agents', async (req, res) => {
    try {
        ...
        const registration = await agentRegistry.registerAgent(agent, type);
...
// Get agent health
app.get('/api/v1/agents/:id/health', async (req, res) => {
    try {
        const agent = await agentRegistry.getAgent(req.params.id);
        if (!agent) {
            return res.status(404).json({ error: 'Agent not found' });
...
app.post('/api/v1/agents/:id/execute', async (req, res) => {
    try {
        ...
        const taskAgent = registration.agent;
...
app.get('/api/v1/agent-types', (req, res) => {
    const types = agentFactory.getAvailableTypes();
    res.json(types);
});
// Integration with Agent Forge
app.post('/api/v1/integrate/agent-forge', async (req, res) => {
    try {
