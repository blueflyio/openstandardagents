...
export class OSSAValidator {
  ...
  private openApiSpec?: OpenAPISchema;
  ...
  async loadOpenAPISpec(specPath: string): Promise<void> {
    try {
      ...
      this.openApiSpec = spec as OpenAPISchema;
    } catch (error) {
   ...
   */
  async validateOpenAPI(specPath: string, options: ValidationOptions = {}): Promise<ValidationResult> {
  ...
  private validateOpenAPIStructure(spec: OpenAPISchema, result: ValidationResult, context: ValidationContext): void {
  ...
  private validateOSSAExtensions(spec: OpenAPISchema, result: ValidationResult, context: ValidationContext): void {
  ...
  private validateSecuritySchemes(spec: OpenAPISchema, result: ValidationResult, context: ValidationContext): void {
  ...
  private validatePaths(spec: OpenAPISchema, result: ValidationResult, context: ValidationContext): void {
  ...
  private validateAgentStructure(agent: Agent, result: ValidationResult, context: ValidationContext): void {
  ...
  private validateAgentCompliance(agent: Agent, result: ValidationResult, context: ValidationContext): void {
  ...
  private validateAgentCapabilities(spec: AgentSpec, result: ValidationResult, context: ValidationContext): void {
  ...
  private validateAgentProtocols(spec: AgentSpec, result: ValidationResult, context: ValidationContext): void {
  ...
  private validateWorkflowStructure(workflow: WorkflowDefinition, result: ValidationResult, context: ValidationContext): void {
  ...
  private validateWorkflowSteps(workflow: WorkflowDefinition, result: ValidationResult, context: ValidationContext): void {
  ...
  private validateWorkflowDependencies(workflow: WorkflowDefinition, result: ValidationResult, context: ValidationContext): void {
  ...
  private validateResponseSchema(response: any, schema: any, result: ValidationResult, context: ValidationContext): void {
  ...
  private validateOSSAResponseFormat(response: any, result: ValidationResult, context: ValidationContext): void {
  ...
  private initializeRules(): void {
    ...
    this.addRules('security', [
      ...
      },
      {
        ...
        severity: 'medium',
        validate: (securitySchemes: any) => {
          const hasApiKey = securitySchemes && Object.values(securitySchemes).some((scheme: any) => 
            scheme.type === 'apiKey'
          );
          return {
            passed: hasApiKey,
    ...
    this.addRules('paths', [
      ...
      },
      {
        ...
        severity: 'medium',
        validate: ({ spec }: { path: string, spec: any }) => {
          const methods = ['get', 'post', 'put', 'delete', 'patch'];
          let hasSchemas = true;
          
          methods.forEach(method => {
            if (spec[method]?.responses) {
              Object.values(spec[method].responses).forEach((response: any) => {
    ...
    this.addRules('agent-structure', [
      {
        ...
        validate: (agent: Agent) => {
          ...
          const missing = required.filter(field => !agent[field as keyof Agent]);
          return {
    ...
    this.addRules('agent-compliance', [
      ...
      },
      {
        ...
        severity: 'medium',
        validate: (agent: Agent) => {
          if (!agent.spec.category) return { passed: true };
          return {
    ...
    this.addRules('agent-capabilities', [
      ...
      },
      {
        ...
        validate: (capabilities: any) => {
          ...
          const invalidNames = (capabilities?.primary || []).filter((cap: string) => !kebabRegex.test(cap));
          return {
    ...
    this.addRules('workflow-structure', [
      {
        ...
        validate: (workflow: WorkflowDefinition) => {
          ...
          const missing = required.filter(field => !workflow[field as keyof WorkflowDefinition]);
          return {
            passed: missing.length === 0,
            message: missing.length > 0 ? `Missing required fields: ${missing.join(', ')}` : undefined,
            suggestion: 'Ensure all required fields are present'
  ...
  private addRules(category: string, rules: ValidationRule[]): void {
  ...
  private createValidationResult(type: string): ValidationResult {
    return {
      valid: true,
      score: 0,
      errors: [],
  ...
  private addRuleResult(rule: ValidationRule, ruleResult: ValidationRuleResult, result: ValidationResult, path?: string): void {
...
export class ValidationFormatter {
  static formatResult(result: ValidationResult, format: 'detailed' | 'summary' | 'json' = 'detailed'): string {
    switch (format) {
      case 'json':
  ...
  private static formatSummary(result: ValidationResult): string {
    const status = result.valid ? '✅ VALID' : '❌ INVALID';
    return `${status} | Score: ${result.score}% | Errors: ${result.errors.length} | Warnings: ${result.warnings.length}`;
  }

  private static formatDetailed(result: ValidationResult): string {
    let output = '';
    ...
    if (result.warnings.length > 0) {
      ...
      result.warnings.forEach((warning, index) => {
        ...
        if (warning.path) output += `     Path: ${warning.path}\n`;
