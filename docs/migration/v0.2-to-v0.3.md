...
EXECUTION_PROFILE=fast ossa run creative-agent-naming.yaml       # Quick responses
EXECUTION_PROFILE=balanced ossa run creative-agent-naming.yaml   # Default
EXECUTION_PROFILE=deep ossa run creative-agent-naming.yaml       # Complex reasoning
EXECUTION_PROFILE=safe ossa run creative-agent-naming.yaml       # Compliance mode
```
...
2. **What the tool does:**
   - ✅ Updates `apiVersion: ossa/v0.2.9` → `ossa/v0.3.0`
   - ✅ Renames `spec.security` → `spec.safety`
   - ✅ Moves `extensions.a2a` → `spec.messaging`
   - ✅ Converts `spec.runtimes` → `runtime`
   - ⚠️ Flags deprecated extensions for manual review
...
#### 3.1 Migrate `spec.security` → `spec.safety`
...
1. **Validate all migrated manifests:**
   ...
   # Expected output:
   # ✓ creative-agent-naming.ossa.yaml (valid)
   # ✗ old-agent.ossa.yaml (3 errors)
   ```

2. **Fix validation errors:**
   ```bash
   # Show detailed errors
   ossa validate --strict --verbose creative-agent-naming.ossa.yaml
   ```

   Common errors:
   ```
   ❌ Error: Unknown field 'spec.security'
      → Fix: Rename to 'spec.safety'

   ❌ Error: Invalid apiVersion 'ossa/v0.2.9'
      → Fix: Update to 'ossa/v0.3.0'

   ❌ Error: Deprecated extension 'extensions.langchain'
      → Fix: Convert to standard tools
   ```

3. **Test agents in development:**
   ```bash
   # Test each agent interactively
   ossa run creative-agent-naming.ossa.yaml --interactive

   # Test with sample input
   echo '{"content_id": "123"}' | ossa run creative-agent-naming.ossa.yaml --input -

   # Test with specific profile
   EXECUTION_PROFILE=fast ossa run creative-agent-naming.ossa.yaml
   ```
...
# ──────────────────────────────────────────────────────────────────
ossa migrate ./creative-agent-naming.ossa.yaml --from v0.2.9 --to v0.3.0 --dry-run

# Expected output:
# 📝 Migration Preview (creative-agent-naming.ossa.yaml)
# ─────────────────────────────────────────────────────────────────
# ✓ Update apiVersion: ossa/v0.2.9 → ossa/v0.3.0
# ✓ Rename spec.security → spec.safety
# ⚠ Warning: extensions.langchain is deprecated
# ⚠ Warning: spec.runtimes is deprecated
# ─────────────────────────────────────────────────────────────────
...
# STEP 2: Review diff
# ──────────────────────────────────────────────────────────────────
ossa migrate ./creative-agent-naming.ossa.yaml --from v0.2.9 --to v0.3.0 --diff
...
# ──────────────────────────────────────────────────────────────────
ossa migrate ./creative-agent-naming.ossa.yaml --from v0.2.9 --to v0.3.0

# Expected output:
# ✓ Migrated creative-agent-naming.ossa.yaml
# ✓ Backup created: creative-agent-naming.ossa.yaml.bak
# ✓ Validation passed
...
- ✅ **Validation** - Ensures output is valid v0.3.0
- ✅ **Batch processing** - Migrate entire directories
- ✅ **Rollback support** - Restore from backups if needed
- ✅ **Git integration** - Works with version control
- ⚠️ **Manual review required** - Complex migrations need human oversight
...
**Error:**
```
❌ ValidationError: Unknown field 'spec.security'
   File: creative-agent-naming.ossa.yaml:15
...
**Automated Fix:**
```bash
sed -i 's/  security:/  safety:/g' creative-agent-naming.ossa.yaml
...
**Warning:**
```
⚠ Warning: extensions.langchain is deprecated in v0.3.0
   File: creative-agent-naming.ossa.yaml:42
...
**Error:**
```
❌ RuntimeError: No messaging transport configured
...
**Error:**
```
❌ RuntimeError: Capability 'publish_content' not bound to runtime handler
...
❌ WorkflowError: Circular dependency detected
   ...
   Steps: fetch-content → ai-review → fetch-content
```
...
steps:
  - id: fetch-content
    depends_on: [ai-review]  # ❌ Creates cycle
  - id: ai-review
...
steps:
  ...
  - id: ai-review
    depends_on: [fetch-content]  # ✓ Linear dependency
```
...
**Error:**
```
❌ LLMConfigError: Provider 'claude' not recognized
...
llm:
  provider: claude    # ❌ Not standard

# After (v0.3.0) - official names
llm:
  provider: anthropic # ✓ Use official provider name
  model: claude-sonnet-4-20250514
```
...
ossa validate --strict creative-agent-naming.ossa.yaml

# What strict mode checks:
# ✓ Schema compliance (v0.3.0)
...
```
✅ SUCCESS: creative-agent-naming.ossa.yaml is valid
──────────────────────────────────────────────────────────────
✓ Schema: v0.3.0 (valid)
✓ Kind: Agent (valid)
✓ Metadata: All required fields present
✓ LLM Config: anthropic/claude-sonnet-4-20250514 (valid)
✓ Tools: 3 tools (all valid)
✓ Safety: Content filtering + PII detection enabled
✓ Messaging: 2 channels published, 1 subscribed

ℹ INFO: No warnings or recommendations
──────────────────────────────────────────────────────────────
```
...
```
❌ ValidationError: Missing required field 'metadata.name'
...
```yaml
metadata:
  name: creative-agent-naming  # Required
  version: 1.0.0
```
...
```
❌ ValidationError: Invalid value 'claude' for field 'llm.provider'
...
```
❌ ValidationError: Field 'spec.security' not defined in v0.3.0 schema
...
# Step 3: Gradually shift traffic (10% → 50% → 100%)
ossa traffic --env production-green --weight 10
...
# Step 3: Gradually increase (1% → 10% → 50% → 100%)
...
- ❌ Runtime errors in production (>5% error rate)
- ❌ Performance degradation (>50% latency increase)
- ❌ A2A messaging failures (messages lost)
...
- Rename `spec.security` → `spec.safety` (if used)
- Migrate `extensions.a2a` → `spec.messaging` (if used)
...
**A:** Deprecated extensions still work in v0.3.0 but will be removed in v{{VERSION}}. You should migrate to standard OSSA features:
...
- [ ] Rename `spec.security` → `spec.safety`
- [ ] Migrate `extensions.a2a` → `spec.messaging`
- [ ] Convert `spec.runtimes` → `runtime`
...
|--------------|--------------|-------|------|----------|-----|--------|
| v0.1.9 | 2024-06-01 | ✅ | ❌ | ❌ | ❌ | Deprecated |
| v0.2.0 | 2024-08-01 | ✅ | ❌ | ❌ | ⚠️ | Deprecated |
| v0.2.9 | 2024-11-01 | ✅ | ❌ | ❌ | ⚠️ | Supported until 2025-06-01 |
| **v0.3.0** | **2025-01-01** | ✅ | ✅ | ✅ | ✅ | **Current** |
| v{{VERSION}} | 2025-Q2 | ✅ | ✅ | ✅ | ✅ | Planned |
| v1.0.0 | 2025-Q4 | ✅ | ✅ | ✅ | ✅ | Roadmap |

**Legend:**
- ✅ Fully supported
- ⚠️ Partial support (via extensions)
- ❌ Not supported

---

## Appendix C: Schema Field Mapping (v0.2.9 → v0.3.0)
