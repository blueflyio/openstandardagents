...
Upgrade to OSSA v0.4.1 for production-quality exports, API endpoints, and cost optimization.

## Overview

v0.4.1 is a major feature release focused on production readiness:
...
None! v0.4.1 is fully backward compatible with v0.4.0 manifests.
...
```bash
# Update to v0.4.1
npm install @bluefly/openstandardagents@0.4.1
...
# Install v0.4.1
npm install -g @bluefly/openstandardagents@0.4.1
...
```bash
# Validate v0.4.0 manifests work with v0.4.1
ossa validate agent.ossa.yaml

# Should output:
# ✓ Valid OSSA manifest (v0.4.0)
# ℹ Compatible with v0.4.1
```

### 2. Update API Version (Optional)

Update to v0.4.1 to access new features:
...
# After (v0.4.1) - optional, v0.4.0 still works
...
```bash
# Auto-upgrade to v0.4.1 format
ossa standardize agent.ossa.yaml --target-version v0.4.1

# Creates agent.ossa.yaml.v0.4.1
...
# After (v0.4.1) - add API endpoints
...
```yaml
apiVersion: ossa/v0.4.1  # Updated
kind: Agent
metadata:
  name: code-reviewer
...
# Verify
ossa --version  # Should show 0.4.1
```
...
All v0.4.0 manifests work with v0.4.1 without changes:
...
None. All v0.4.1 APIs are additions, not replacements.
