...
import { TemplateProcessorService, TemplateVariables } from '../template-processor.service.js';
...
export class RepoAgentsMdService {
  constructor(
    @inject(TemplateProcessorService) private templateProcessor: TemplateProcessorService,
   ...
   */
  async generate(config: RepoConfig & { template_overrides?: any, auto_commit?: boolean, auto_push?: boolean }): Promise<GenerationResult> {
    try {
      // 1. Extract variables from repo
      const extractedVars = await this.templateProcessor.extractVariablesFromRepo(config.repo_path);
      ...
      const variables: TemplateVariables = {
        ...
        ...(config.description ? { PROJECT_DESCRIPTION: config.description } : {}),
      ...
      return {
        ...
        content: '',
        error: error.message,
        validation: { valid: false, follows_standard: false, errors: [error.message], warnings: [] }
      };
    }
  }
   ...
   */
  async batchGenerate(repos: RepoConfig[], parallel: boolean = true): Promise<{ results: GenerationResult[], summary: any }> {
    const results: GenerationResult[] = [];
    
    if (parallel) {
      const promises = repos.map(repo => this.generate({ ...repo, auto_commit: true, auto_push: true }));
      results.push(...(await Promise.all(promises)));
    } else {
      for (const repo of repos) {
        results.push(await this.generate({ ...repo, auto_commit: true, auto_push: true }));
      }
    }
  ...
  async validate(content: string): Promise<ValidationResult> {
    ...
    if (placeholders) {
      warnings.push(`Contains unreplaced placeholders: ${placeholders.join(', ')}`);
    }
   ...
   */
  async delete(repoPath: string, auto_commit: boolean = false): Promise<any> {
    const filePath = path.join(repoPath, 'AGENTS.md');
    try {
