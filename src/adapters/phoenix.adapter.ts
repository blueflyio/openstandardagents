...
export class PhoenixAdapter {
  ...
  async initialize(
    ...
    const apiKey = config.api_key || 
                   (config.api_key_env ? process.env[config.api_key_env] : process.env.PHOENIX_API_KEY);

    if (!apiKey) {
      throw new Error('Phoenix API key not found. Set PHOENIX_API_KEY environment variable.');
    }
