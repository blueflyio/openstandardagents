...
import { getTemplateChoices, getTemplate, type AgentTemplate } from '../templates/agent-templates.js';
=======
...
<<<<<<< HEAD
  .option(
    '--generate-agents-md',
    'Automatically generate AGENTS.md documentation',
    false
  )
  .action(
    async (options?: {
      ...
      try {
        ...
        // ========================================================================
        printStep(0, 10, 'Choose Agent Template');

        printInfo('Select a pre-configured template to get started quickly.');
        printInfo('Templates provide default values for common agent patterns.\n');

        const templateId = await askChoice(
        ...
        console.log('');
        console.log(chalk.green(`✓ Selected: ${template.icon} ${template.name}`));
        console.log(chalk.gray(`  ${template.description}`));
        console.log('');
        console.log(chalk.cyan('Example use cases:'));
        template.useCases.forEach((useCase, i) => {
          console.log(chalk.gray(`  ${i + 1}. ${useCase}`));
        });
        console.log('');
        console.log(chalk.cyan('Example agents:'));
        ...
        // ========================================================================
        printStep(1, 10, 'Basic Agent Information');
        ...
        // ========================================================================
        printStep(2, 10, 'Agent Role & System Prompt');
        ...
        printInfo(chalk.gray(`Template default: ${template.defaults.role}\n`));

        const role =
          (await question(
            chalk.blue('System Prompt / Role (press Enter to use template default):\n') +
              chalk.gray(
                '  (Or enter custom role description)\n'
              )
        ...
        // ========================================================================
        printStep(3, 10, 'LLM Configuration');

        printInfo(chalk.gray(`Template default: ${template.defaults.llm?.provider} (${template.defaults.llm?.model})\n`));
        ...
        const llmProvider = await askChoice(
          ...
          template.defaults.llm?.provider || 'anthropic'
        );
        ...
        const temperature =
          (await question(
            chalk.blue('Temperature (0.0-2.0, default: 0.7): ')
          )) || '0.7';
        ...
        // STEP 4: Tools
        // ========================================================================
        printStep(4, 10, 'Tools & Capabilities');

        printInfo('Tools allow your agent to interact with external systems.');
        printInfo(
          'You can add MCP servers, function tools, or skip for now.\n'
        );
        printInfo(chalk.gray(`Template suggests: ${template.defaults.tools.join(', ')}\n`));
        ...
        // ========================================================================
        printStep(5, 10, 'Safety & Security Controls');
        ...
        // ========================================================================
        printStep(6, 10, 'Autonomy & Human-in-the-Loop');

        printInfo(
          'Configure how autonomous the agent should be and when human approval is required.\n'
        );
        printInfo(chalk.gray(`Template default: ${template.defaults.autonomy}\n`));
        ...
        // Apply template autonomy defaults
        const autonomy: any = {
          level: template.defaults.autonomy,
          requires_approval: template.defaults.autonomy === 'supervised',
        };
        ...
        // ========================================================================
        printStep(7, 10, 'Observability & Monitoring');
        ...
        // ========================================================================
        printStep(8, 10, 'Platform Extensions');
...
<<<<<<< HEAD
        ...
        // Generate AGENTS.md if requested or confirmed
        const shouldGenerateMd =
          options?.generateAgentsMd ||
          (await askYesNo(
            'Do you want to generate AGENTS.md documentation for this agent?',
            true
          ));

        if (shouldGenerateMd) {
          try {
            const repoAgentsMdService = container.get(RepoAgentsMdService);
            const repoPath = useDirectory
              ? path.dirname(outputPath)
              : process.cwd();

            await repoAgentsMdService.generate({
              repo_path: repoPath,
              project_name: agentName,
              description: description,
            });

            console.log(chalk.green('✓ AGENTS.md documentation generated'));
          } catch (err) {
            console.log(
              chalk.yellow(
