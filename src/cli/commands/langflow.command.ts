...
langflowCommand
  ...
  .option('-o, --output <path>', 'Output path for Langflow flow JSON', 'flow.json')
...
langflowCommand
  ...
  .option('--api-key <key>', 'Langflow API key (or set LANGFLOW_API_KEY env var)')
  .option('--input <json>', 'Input JSON string', '{}')
  .action(async (manifestFile: string, options: { baseUrl: string; apiKey?: string; input: string }) => {
    ...
    try {
      ...
      if (!extensions?.flow_id) {
        console.error(chalk.red('❌ Langflow flow_id not found in manifest extensions'));
        process.exit(1);
      }
