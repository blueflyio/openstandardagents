...
workspaceCommand
  ...
  .action(async (options) => {
    try {
      ...
      const registryFilePath = getWorkspaceRegistryPath();
      const registryContent = yaml.stringify({
        apiVersion: getDefaultOSSAAPIVersion(),
        kind: getDefaultRegistryKind(),
        metadata: {
        ...
        agents: [],
        discovery: {
          strategy: getDefaultDiscoveryStrategy(),
          refresh: getDefaultDiscoveryRefresh(),
          patterns: getDefaultDiscoveryPatterns(),
        },
      });
      fs.writeFileSync(
        path.join(workspaceDir, registryFilePath),
      ...
      fs.writeFileSync(
        path.join(workspaceDir, policyFilePath),
...
`;
      ...
      console.log(chalk.gray(`  2. Edit ${getWorkspacePolicyPath()} to configure allowed tools`));
  ...
  .action(async (options) => {
    try {
        ...
        for (const dir of requiredDirs) {
          ...
          if (fs.existsSync(dirPath)) {
            ...
            validationResults.push({
              ...
              details: `Create: mkdir -p .agents-workspace/${dir.name}`,
            });
          } else {
            validationResults.push({
              category: 'Structure',
      ...
      if (summary.failed === 0) {
        ...
        console.log(chalk.red('\n✗ Workspace structure has validation errors'));
      }
