...
export const publishCommand = new Command('publish')
  ...
  .action(async (manifestPath: string, options: { registry?: string; token?: string; dryRun?: boolean }) => {
    try {
      ...
      const token = options.token || process.env.GITLAB_TOKEN || process.env.GITLAB_PRIVATE_TOKEN || process.env.GITHUB_TOKEN;
      if (!token) {
      ...
      try {
        await axios.post(`${gitlabUrl}/api/v4/projects/${projectId}/releases`, {
