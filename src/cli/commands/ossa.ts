...
async function enforceTDD(taskId: string, options: any): Promise<void> {
  ...
  try {
    ...
    console.log(chalk.gray(`   Red-Green-Refactor: ${DEFAULT_OSSA_CONFIG.tddEnforcement.enforceRedGreenRefactor ? 'ENFORCED' : 'OPTIONAL'}`));
...
async function showStatus(): Promise<void> {
  ...
  console.log(`   Executions: ${chalk.blue(health.executions.active)} active, ${chalk.gray(health.executions.total)} total`);
