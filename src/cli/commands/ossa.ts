...
import {
  ...
  DEFAULT_OSSA_CONFIG,
  AgentSpawnConfig,
} from '../../core/orchestrator/ossa-orchestrator.js';
...
async function initializeOrchestrator(options: any): Promise<void> {
  ...
  try {
    ...
    console.log(
      chalk.gray(
        `   - Compliance level: ${config.rebuildTools.complianceLevel.toUpperCase()}`
      )
    );
  } catch (error) {
    console.error(
      chalk.red('❌ Failed to initialize OSSA Orchestrator:'),
      error
    );
    process.exit(1);
  }
...
async function rebuildTools(tools: string[], options: any): Promise<void> {
  ...
  try {
    ...
    console.error(
      chalk.red('❌ Failed to execute __REBUILD_TOOLS workflow:'),
      error
    );
    process.exit(1);
  }
...
async function coordinateWorkflow(
  ...
  try {
    ...
    const executionId = await orchestrator.coordinateMultiAgentWorkflow(
      workflow,
      strategy
    );
...
async function enforceTDD(taskId: string, options: any): Promise<void> {
  ...
  try {
    ...
    console.log(
      chalk.gray(
        `   Test coverage required: ${DEFAULT_OSSA_CONFIG.tddEnforcement.minimumCoverage}%`
      )
    );
    console.log(
      chalk.gray(
...
async function showStatus(): Promise<void> {
  ...
  console.log(
    `   Status: ${health.status === 'healthy' ? chalk.green('HEALTHY') : chalk.red('UNHEALTHY')}`
  );
  console.log(
    `   Agents: ${chalk.blue(health.agents.total)} total, ${chalk.green(health.agents.active)} active`
  );
  console.log(
  ...
  if (executions.length > 0) {
    ...
    executions.forEach((exec) => {
      console.log(
        `   ${chalk.blue(exec.id.substring(0, 8))} - ${exec.status.toUpperCase()}`
      );
      console.log(
        `      Phase: ${exec.phases[exec.currentPhase]?.name || 'unknown'}`
      );
      console.log(
        `      Tokens: ${exec.budget.usedTokens}/${exec.budget.totalTokens}`
      );
    });
  }
}
...
program
  .name('ossa')
  .description(
    'OSSA v0.1.9 Orchestrator - Master orchestrator for OSSA-compliant agent systems'
  )
  .version('0.1.9');
...
program
  ...
  .option(
    '-s, --strategy <strategy>',
    'Coordination strategy (parallel|sequential|adaptive)',
    'adaptive'
  )
...
program
  ...
  .action(async (agentId: string) => {
    ...
    try {
      ...
      console.log(
        chalk.green(`✅ Lifecycle management started for agent ${agentId}`)
      );
    } catch (error) {
