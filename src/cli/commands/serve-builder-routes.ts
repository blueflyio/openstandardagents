...
const BUILTIN_SKILLS = [
  {
    ...
    platforms: ['claude-code', 'cursor', 'codex-cli'],
  ...
  },
  {
    ...
    version: '1.0.0',
    description:
      'Review and improve git commit messages following conventional commits',
  ...
  },
  {
    name: 'security-audit',
    version: '1.0.0',
    description: 'OWASP Top 10 security scanning and vulnerability detection',
    categories: ['security', 'compliance'],
    platforms: ['claude-code', 'cursor', 'codex-cli'],
    allowedTools: ['bash', 'file_read', 'web_search'],
  },
  {
    name: 'api-design',
    version: '1.0.0',
    description:
      'OpenAPI 3.1 spec design and validation with Zod type generation',
  ...
  },
  {
    ...
    platforms: ['claude-code', 'cursor', 'codex-cli', 'kiro'],
  ...
  },
  {
    ...
    version: '1.0.0',
    description:
      'Drupal module development with PHPCS, PHPStan, and hook validation',
    categories: ['drupal', 'php', 'development'],
    platforms: ['claude-code', 'cursor'],
    allowedTools: ['bash', 'file_read', 'file_write'],
  },
  {
    ...
    version: '1.0.0',
    description:
      'React component patterns, hooks, and performance optimization',
    categories: ['frontend', 'react', 'development'],
    platforms: ['claude-code', 'cursor', 'codex-cli', 'windsurf'],
  ...
  },
  {
    ...
    version: '1.0.0',
    description:
      'Kubernetes deployment, scaling, health checks, and troubleshooting',
  ...
  },
  {
    name: 'ci-pipeline',
    version: '1.0.0',
    description: 'CI/CD pipeline configuration for GitLab CI, GitHub Actions',
  ...
  },
  {
    ...
    version: '1.0.0',
    description: 'Create and package Model Context Protocol servers',
    categories: ['mcp', 'tooling', 'development'],
    platforms: ['claude-code', 'cursor', 'codex-cli'],
    allowedTools: ['bash', 'file_read', 'file_write', 'web_search'],
  },
];
...
export async function handleBuilderRoute(
  ...
  if (pathname === '/api/skills/attach' && req.method === 'POST') {
    ...
    const manifestYaml =
      typeof body.manifestYaml === 'string' ? body.manifestYaml : '';

    if (!skillName)
      return (json(res, 400, { error: 'skillName is required' }), true);
    if (!manifestYaml)
      return (json(res, 400, { error: 'manifestYaml is required' }), true);
    ...
    try {
      ...
      if (refs.includes(skillName)) {
        json(res, 200, {
          manifestYaml,
          skillRefs: refs,
          message: 'Skill already attached',
        });
        return true;
      }
      ...
      manifest.extensions = {
        ...ext,
        skills: { ...skillsExt, skillRefs: refs },
      };
  ...
  if (pathname === '/api/agent-builder' && req.method === 'POST') {
    ...
    const exportTriggerToken =
      process.env.EXPORT_TRIGGER_TOKEN || process.env.GITLAB_TOKEN;
    ...
    if (exportTriggerToken && (manifestYaml ?? '').trim().length > 0) {
      const platforms: string[] =
        Array.isArray(platformsBody) && platformsBody.length > 0
          ? platformsBody
          : [platform ?? 'docker'].filter(Boolean);
      const safeName =
        (agentName ?? 'my-agent')
          .replace(/[^a-zA-Z0-9_-]/g, '-')
          .slice(0, 64) || 'my-agent';
      const base64 = Buffer.from(String(manifestYaml).trim(), 'utf-8').toString(
        'base64'
      );
      try {
        ...
        if (platforms.length > 1)
          form.set('variables[EXPORT_PLATFORMS]', platforms.join(','));
        ...
        const webUrl =
          pipeline?.web_url ??
          `https://gitlab.com/${decodeURIComponent(exportProjectPath)}/-/pipelines`;
        if (!triggerRes.ok) {
          json(res, triggerRes.status >= 400 ? triggerRes.status : 502, {
            ...
            steps: [
              {
                step: 'trigger',
                command: 'POST trigger/pipeline',
                stdout: '',
                stderr: JSON.stringify(pipeline ?? {}),
                exitCode: 1,
              },
            ],
        ...
        json(res, 200, {
          ...
          steps: [
            {
              step: 'trigger',
              command: 'Triggered pipeline',
              stdout: `Pipeline ${pipeline?.id ?? 'started'}: ${webUrl}`,
              stderr: '',
              exitCode: 0,
            },
          ],
        ...
        json(res, 502, {
          error: 'Export pipeline trigger failed',
          details: err.message,
        });
        return true;
      }
    ...
    if (invalid.length > 0) {
      json(res, 400, {
        error: `Invalid platform(s): ${invalid.join(', ')}. Valid: ${PLATFORMS.join(', ')}`,
      });
      return true;
    }

    const safeName =
      (agentName || 'my-agent').replace(/[^a-zA-Z0-9_-]/g, '-').slice(0, 64) ||
      'my-agent';
    ...
    try {
      ...
      if (manifestYaml) {
        ...
        steps.push({
          step: 'write-manifest',
          command: `# Manifest → ${safeName}.ossa.yaml`,
          stdout: `Wrote ${manifestYaml.length} bytes`,
          stderr: '',
          exitCode: 0,
        });
      } else {
      ...
      for (const plat of platforms) {
        ...
        try {
          ...
          steps.push({
            step: `export:${plat}`,
            command: `ossa export --platform ${plat}`,
            stdout: r.stdout,
            stderr: r.stderr,
            exitCode: 0,
          });
  ...
  if (pathname === '/api/agent-builder/save' && req.method === 'POST') {
    ...
    if (!saveUrl) {
      json(res, 503, {
        error: 'Save not configured. Set OSSA_WEBAGENTS_API_URL.',
      });
      return true;
    }
    const body = JSON.parse((await readBody(req)) || '{}');
  ...
  if (kind === 'Agent' && mode === 'quick') {
    return [
        ...
        fields: [
          {
            ...
            label: 'Agent Name',
            required: true,
            description: 'Unique identifier (lowercase, hyphens ok)',
          },
          {
            name: 'description',
            type: 'textarea',
      ...
      },
      {
        id: 'type',
        title: 'Agent Type',
        description: 'Select the agent role',
        fields: [
          {
            name: 'agentType',
            type: 'select',
            ...
            options: [
              ...
              },
              {
                label: 'Monitor',
                value: 'monitor',
                description: 'Observes and reports',
              },
              {
                label: 'Gateway',
                value: 'gateway',
      ...
      },
      {
        ...
        description: 'Where will this agent run?',
        fields: [
          {
            name: 'platforms',
            type: 'multiselect',
  ...
  if (kind === 'Agent' && mode === 'guided') {
    return [
        ...
        fields: [
          {
            ...
            description: 'e.g. code-review, deploy, test',
          },
          {
            name: 'allowedTools',
            type: 'multiselect',
      ...
      },
      {
        id: 'skills',
        title: 'Skills',
        description: 'Attach reusable skills',
        fields: [
          {
            name: 'skills',
            type: 'multiselect',
            label: 'Skills',
            options: BUILTIN_SKILLS.map((s) => ({
              label: s.name,
              value: s.name,
