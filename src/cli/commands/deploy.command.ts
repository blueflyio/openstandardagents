...
deployGroup.command('promote')
  ...
  .action(async (manifestPath: string, options: { from?: string; to?: string }) => {
    try {
      ...
      console.error(chalk.red('✗ Promotion failed:'), error instanceof Error ? error.message : String(error));
      process.exit(1);
    }
  });

deployGroup.command('status')
  .argument('<path>', 'Path to agent manifest')
  ...
  .action(async (manifestPath: string, options: { env?: string }) => {
    try {
      ...
      if (options.env) {
        ...
        console.log(chalk.white(`  Deployed:    ${env.deployedAt ? new Date(env.deployedAt).toLocaleString() : 'N/A'}`));
      } else {
...
deployGroup.command('retire')
  ...
  .action(async (manifestPath: string, options: { effective?: string; replacement?: string }) => {
    try {
      ...
      console.error(chalk.red('✗ Retirement failed:'), error instanceof Error ? error.message : String(error));
      process.exit(1);
    }
  });
