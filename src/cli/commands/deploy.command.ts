...
const deployGroup = new Command('deploy').description(
  'Deploy and lifecycle management'
);
    ...
    async (
      ...
      try {
        ...
        if (!manifest.metadata || !manifest.spec) {
          console.error(
            chalk.red('✗ Invalid manifest: missing metadata or spec')
          );
          process.exit(1);
        }
        ...
        console.log(
          chalk.green(`✓ Deployed ${agentName}@${version} to ${env}`)
        );
      } catch (error) {
        console.error(
...
deployGroup
  ...
  .action(
    async (manifestPath: string, options: { from?: string; to?: string }) => {
      try {
        const manifestRepo = container.get(ManifestRepository);
...
deployGroup
  ...
  .action(
    async (
      manifestPath: string,
      options: { effective?: string; replacement?: string }
    ) => {
      try {
        const manifestRepo = container.get(ManifestRepository);
        const manifest = await manifestRepo.load(manifestPath);
        ...
        if (options.effective) {
          ...
          manifest.metadata.lifecycle.deprecation.sunsetDate =
            options.effective;
        }
        if (options.replacement) {
          ...
          manifest.metadata.lifecycle.deprecation.replacement =
            options.replacement;
        }
        await manifestRepo.save(manifestPath, manifest);
