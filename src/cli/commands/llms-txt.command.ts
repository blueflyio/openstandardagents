...
export const llmsTxtCommand = new Command('llms-txt').description(
  'Generate, validate, and sync llms.txt files from OSSA manifests'
);
    ...
    async (
      ...
      try {
        ...
        if (!extension?.enabled) {
          console.error(
            chalk.red('Error: llms_txt extension is not enabled in manifest')
          );
          console.log(
...
`)
        ...
        console.log(
          chalk.gray(
            `  2. Validate: ossa llms-txt validate ${options.output} ${manifestPath}`
          )
        );
  ...
  .action(
    async (
      ...
      try {
        ...
        const manifest = await manifestRepo.load(manifestPath);
        const result = await llmsTxtService.validateLlmsTxt(
          llmsTxtPath,
          manifest
        );
