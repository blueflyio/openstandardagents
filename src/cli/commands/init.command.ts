...
export const initCommand = new Command('init')
  ...
  .action(async (name?: string, options?: { output?: string; yes?: boolean }) => {
    try {
      ...
      if (!useDefaults) {
        ...
        agentDisplayName = (await question(chalk.blue('Agent Display Name: '))) || agentName;
        description = (await question(chalk.blue('Description: '))) || '';
        version = (await question(chalk.blue('Version (default: 1.0.0): '))) || '1.0.0';
        role = (await question(chalk.blue('Agent Role/System Prompt: '))) || '';
        llmProvider =
          (await question(
            chalk.blue('LLM Provider (openai/anthropic/google, default: openai): ')
          )) || 'openai';
        llmModel =
          (await question(
            chalk.blue(
        ...
        const platformsInput =
          (await question(
            chalk.blue('Target Platforms (comma-separated: cursor,openai,crewai,langchain,etc): ')
          )) || '';
      ...
      const manifest: OssaAgent = {
        ...
        metadata: {
          name: agentName,
          version: version,
          description: description || `${agentDisplayName} - OSSA-compliant agent`,
        },
        spec: {
      ...
      console.log(chalk.green(`✓ Created OSSA agent manifest: ${outputPath}`));
