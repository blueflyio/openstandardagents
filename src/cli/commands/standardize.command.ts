...
export const standardizeCommand = new Command('standardize')
  ...
  .action(async (paths: string[] = [], options?: {
    ...
    try {
      ...
      console.log(chalk.blue(`Standardizing ${manifestFiles.length} manifest(s)...`));
      if (options?.dryRun) {
        ...
        try {
          ...
          if (currentModel && !currentModel.includes('${')) {
            ...
            if (!options?.dryRun) {
              ...
              if (!manifest.spec) {
                const role = safeGet<string>(manifest.spec, 'role', (v): v is string => typeof v === 'string') || getDefaultRoleTemplate(safeGet<string>(manifest.metadata, 'name', (v): v is string => typeof v === 'string') || 'agent');
                manifest.spec = { role };
              }
              if (!manifest.spec.llm || typeof manifest.spec.llm !== 'object') {
                const provider = safeGet<string>(manifest.spec.llm, 'provider', (v): v is string => typeof v === 'string') || getDefaultLLMProvider();
          ...
          console.log(chalk.red(`✗ Failed to process ${file}: ${error.message}`));
        }
      }
      ...
      const appliedFixes = allFixes.reduce((sum, f) => sum + f.fixes.filter(fix => fix.applied).length, 0);

      if (options?.dryRun) {
        console.log(chalk.yellow(`Would apply ${totalFixes} fix(es) to ${allFixes.length} manifest(s)`));
        console.log(chalk.gray('  Run without --dry-run to apply fixes'));
      } else {
        console.log(chalk.green(`✓ Applied ${appliedFixes} fix(es) to ${allFixes.length} manifest(s)`));
      }
