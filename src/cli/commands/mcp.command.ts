...
bridgeCommand
  ...
  .action(async (source: string, options: { dir: string }) => {
    try {
      ...
      const { McpBridgeService } =
        await import('../../services/mcp/bridge.service.js');
      ...
      if (result.servers.length > 0) {
        ...
        for (const s of result.servers) {
          ...
          if (s.tools && s.tools.length > 0) {
            console.log(
              chalk.gray(`      tools (via SDK): ${s.tools.join(', ')}`)
            );
          }
        }
      } else {
  ...
  .action(async (options: { dir: string }) => {
    try {
      ...
      if (result.servers.length === 0) {
        ...
        console.log(
          chalk.gray(`  Run: ossa mcp bridge sync <cursor|claude-desktop>`)
        );
        return;
      }
...
bridgeCommand
  ...
  .description(
    'Validate whether OSSA policy allows agentId to call toolName (format: serverName/method)'
  )
  .option('--dir <directory>', 'Base workspace directory', '.')
  .action(
    async (agentId: string, toolName: string, options: { dir: string }) => {
      try {
        const { container } = await import('../../di-container.js');
        const { McpBridgeService } =
          await import('../../services/mcp/bridge.service.js');
        ...
        console.log(
          result.allowed ? chalk.green(`✓ ALLOWED`) : chalk.red(`✗ DENIED`)
        );
