...
class GoldenValidator {
  ...
  private getWorkingBranchCount(): number {
    try {
      ...
      const workingBranches = branches
        .split('\n')
        .filter((branch) =>
          /^\s*(feature|bug|chore|docs|hotfix|test|perf|ci)\//.test(branch)
        );
  ...
  private validateComponentStructure(): void {
    ...
    this.check(
      this.fileExists('.gitlab/components/workflow/golden/component.yml'),
      'Golden component.yml exists'
    );
    this.check(
      this.fileExists('.gitlab/components/workflow/golden/template.yml'),
      'Golden template.yml exists'
    );
    this.check(
      this.fileExists('.gitlab/components/workflow/golden/README.md'),
      'Golden README.md exists'
    );
  }
  ...
  private validateVersionDetection(): void {
    ...
    this.check(
      this.fileContains(
        '.gitlab/components/workflow/golden/component.yml',
        'version: "0.1.0"'
      ),
  ...
  private validateBranchCompliance(): void {
    ...
    this.check(
      branchCount <= 40,
      `Working branches ≤40 (5 per type): ${branchCount}`,
      'warning'
    );
    ...
    this.check(
      currentBranch !== 'main',
      `Not on main branch (current: ${currentBranch})`
    );
  }
  ...
  private validateGitLabCI(): void {
    ...
    this.check(
      this.fileContains('.gitlab-ci.yml', 'ENABLE_OSSA.*true'),
      'OSSA compliance enabled'
    );
    this.check(
      this.fileContains('.gitlab-ci.yml', 'ENABLE_TDD.*true'),
      'TDD compliance enabled'
    );
  }
  ...
  private validatePipelineJobs(): void {
    ...
    this.check(
      this.fileContains(templatePath, 'detect:version:'),
      'Version detection job defined'
    );
    this.check(
      this.fileContains(templatePath, 'test:ai-ml:'),
      'AI/ML testing job defined'
    );
    this.check(
      this.fileContains(templatePath, 'tag:pre-release:'),
      'Pre-release tagging job defined'
    );
    this.check(
      this.fileContains(templatePath, 'changelog:update:'),
      'CHANGELOG update job defined'
    );
    this.check(
      this.fileContains(templatePath, 'release:production:'),
      'Production release job defined'
    );
    this.check(
      this.fileContains(templatePath, 'when: manual'),
      'Manual release gate configured'
    );
    this.check(
      this.fileContains(templatePath, 'inputs.project_version'),
      'Explicit version input supported'
    );
    this.check(
      this.fileContains(templatePath, 'inputs.enable_ai_ml_testing'),
      'AI/ML testing input supported'
    );
    this.check(
      this.fileContains(templatePath, 'inputs.ossa_compliance_check'),
      'OSSA compliance input supported'
    );
  }
  ...
  private validateSafetyChecks(): void {
    ...
    this.check(
      whenNeverCount <= validWhenNever,
      'No problematic "when: never" blockers'
    );
    this.check(
      this.fileContains(templatePath, 'rules:'),
      'Uses rules instead of only/except'
    );
    this.check(
      !this.fileContains(templatePath, 'force-push'),
  ...
  private validateIntegrationPoints(): void {
    ...
    this.check(
      this.fileContains('.gitlab-ci.yml', 'test:integration'),
    ...
    this.check(
      this.fileContains('.gitlab-ci.yml', 'UADP'),
      'UADP testing included',
      'warning'
    );
  }
  ...
  private validateTestingSupport(): void {
    ...
    this.check(
      this.fileContains('package.json', 'test:uadp'),
      'UADP test script defined',
      'warning'
    );
    this.check(
      this.fileContains('package.json', 'test:integration'),
      'Integration test script defined',
      'warning'
    );
    this.check(
      this.fileContains('package.json', 'test:ai-ml'),
      'AI/ML test script defined',
      'warning'
    );
  }
  ...
  private validateDocumentation(): void {
    ...
    this.check(
      this.fileContains('CHANGELOG.md', '0.1.9'),
      'CHANGELOG has v0.1.9 entry'
    );
    this.check(this.fileExists('README.md'), 'README.md exists');
    this.check(this.fileExists('ROADMAP.md'), 'ROADMAP.md exists');
    this.check(
      this.fileExists('docs/gitlab-component-requirements.md'),
      'GitLab component requirements documented'
    );
  }
  ...
  private printSummary(): void {
    ...
    if (this.result.fail === 0) {
      console.log(
        '🎉 Golden CI Orchestration Component is properly configured!'
      );
      process.exit(0);
    } else {
