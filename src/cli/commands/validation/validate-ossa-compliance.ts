...
export async function main() {
  try {
    ...
    // Count total agents
    execSync("TOTAL_AGENTS=$(find \"$PROJECT_ROOT\" -name \"agent.yml\" -type f | wc -l | tr -d ' ')", { stdio: 'inherit' });
    console.log("📊 Total agents found: $TOTAL_AGENTS");

    // Check OpenAPI specs
    execSync("OPENAPI_COUNT=$(find \"$PROJECT_ROOT\" -name \"openapi.yml\" -type f | wc -l | tr -d ' ')", { stdio: 'inherit' });
    console.log("📋 OpenAPI specs: $OPENAPI_COUNT / $TOTAL_AGENTS");

    // Check handlers
    execSync("HANDLER_COUNT=$(find \"$PROJECT_ROOT\" -name \"*.handlers.ts\" -type f | wc -l | tr -d ' ')", { stdio: 'inherit' });
    console.log("🔧 TypeScript handlers: $HANDLER_COUNT / $TOTAL_AGENTS");

    // Check schemas
    execSync("SCHEMA_COUNT=$(find \"$PROJECT_ROOT\" -name \"*.schema.json\" -type f | wc -l | tr -d ' ')", { stdio: 'inherit' });
    console.log("📋 JSON schemas: $SCHEMA_COUNT / $TOTAL_AGENTS");

    // Check metadata
    execSync("METADATA_COUNT=$(find \"$PROJECT_ROOT\" -name \".agents-metadata.json\" -type f | wc -l | tr -d ' ')", { stdio: 'inherit' });
    console.log("📄 OSSA metadata: $METADATA_COUNT / $TOTAL_AGENTS");

    // Check package.json files
    execSync("PACKAGE_COUNT=$(find \"$PROJECT_ROOT/.agents\" -name \"package.json\" -type f | wc -l | tr -d ' ')", { stdio: 'inherit' });
    ...
    execSync("OVERALL_COMPLIANCE=$(( (OPENAPI_COMPLIANCE + HANDLER_COMPLIANCE + SCHEMA_COMPLIANCE + METADATA_COMPLIANCE + PACKAGE_COMPLIANCE) / 5 ))", { stdio: 'inherit' });
    ...
    execSync("declare -a required_dirs=(\"behaviors\" \"data\" \"handlers\" \"integrations\" \"schemas\" \"src\" \"tests\" \"config\" \"deployments\")", { stdio: 'inherit' });
    ...
    execSync("declare -a required_files=(\"openapi.yml\" \"README.md\" \"package.json\" \"tsconfig.json\" \".agents-metadata.json\")", { stdio: 'inherit' });
