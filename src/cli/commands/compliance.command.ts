...
export const complianceCommand = new Command('compliance')
  ...
  .option(
    '--badge <name>',
    'Check specific badge (ossa, security, docs, practices)'
  )
  .action(
    async (
      paths: string[] = [],
      ...
      try {
        ...
        console.log(
          chalk.blue(
            `Checking compliance for ${manifestFiles.length} manifest(s)...`
          )
        );
          ...
          try {
            ...
            if (!options?.badge || options.badge === 'security') {
              ...
              if (model && !model.includes('${')) {
                securityIssues.push(
                  'Hardcoded model name (should use environment variable)'
                );
              ...
              for (const tool of tools) {
                ...
                if (apiKey) {
                  securityIssues.push(
                    'Hardcoded API key in tool configuration'
                  );
                  securityStatus = 'fail';
                }
              }
            ...
            if (!options?.badge || options.badge === 'docs') {
              ...
              const description = safeGet<string>(
                manifest.metadata,
                'description',
                (v): v is string => typeof v === 'string'
              );
            ...
            console.log(
              chalk.red(`✗ Failed to check ${file}: ${error.message}`)
            );
          }
        }
        ...
        // Summary
        console.log('');
        const totalBadges = allResults.reduce(
          (sum, r) => sum + r.badges.length,
          0
        );
        const passedBadges = allResults.reduce(
          (sum, r) => sum + r.badges.filter((b) => b.status === 'pass').length,
          0
        );
        const failedBadges = allResults.reduce(
          (sum, r) => sum + r.badges.filter((b) => b.status === 'fail').length,
          0
        );
        const warningBadges = allResults.reduce(
          (sum, r) =>
            sum + r.badges.filter((b) => b.status === 'warning').length,
          0
        );
