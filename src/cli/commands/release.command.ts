...
versionCommand
  .command('sync')
  .description('Sync version to all files with 0.3.2 placeholders')
  .option('--dry-run', 'Show what would change without making changes')
...
versionCommand
  ...
  .action(async () => {
    try {
      ...
      // Check package.json
      const packageJson = JSON.parse(fs.readFileSync('package.json', 'utf-8'));
      if (packageJson.version !== version && packageJson.version !== '0.3.2') {
        issues.push(`package.json has version ${packageJson.version}, expected ${version} or 0.3.2`);
      }

      // Check for stray 0.3.2 placeholders in built files
      // Skip version utility files that intentionally reference 0.3.2 to detect it
      ...
      for (const file of builtFiles) {
        // Skip files that intentionally check for 0.3.2 placeholder
        if (versionUtilityFiles.some(util => file.endsWith(util.replace('dist/', '')))) {
          continue;
        }
        const content = fs.readFileSync(file, 'utf-8');
        if (content.includes('0.3.2')) {
          issues.push(`${file} contains unresolved 0.3.2 placeholder`);
        }
      }
...
versionCommand
  ...
  .action(async () => {
    try {
      ...
      const isPlaceholder = packageJson.version === '0.3.2';
      if (isPlaceholder) {
        console.log(chalk.green('\n   ✅ Using 0.3.2 placeholder (correct for CI)'));
