...
export async function main() {
  try {
    ...
    execSync("mv \"$dir\" \"$ARCHIVE_DIR/\" 2>/dev/null || echo -e \"    ${RED}Failed to move $name${NC}\"", { stdio: 'inherit' });
    ...
    execSync("mv \"$DOCS_DIR/01-project-overview.md\" \"$DOCS_DIR/overview/project-overview.md\" 2>/dev/null || true", { stdio: 'inherit' });
    execSync("fi", { stdio: 'inherit' });
    ...
    execSync("mv \"$DOCS_DIR/03-features.md\" \"$DOCS_DIR/overview/features.md\" 2>/dev/null || true", { stdio: 'inherit' });
    execSync("fi", { stdio: 'inherit' });

    execSync("if [ -f \"$DOCS_DIR/04-best-practices.md\" ]; then", { stdio: 'inherit' });
    console.log("-e   ${YELLOW}→ Moving best practices${NC}");
    execSync("mv \"$DOCS_DIR/04-best-practices.md\" \"$DOCS_DIR/development/best-practices.md\" 2>/dev/null || true", { stdio: 'inherit' });
    execSync("fi", { stdio: 'inherit' });

    execSync("if [ -f \"$DOCS_DIR/05-architecture.md\" ]; then", { stdio: 'inherit' });
    console.log("-e   ${YELLOW}→ Moving architecture docs${NC}");
    execSync("mv \"$DOCS_DIR/05-architecture.md\" \"$DOCS_DIR/overview/architecture.md\" 2>/dev/null || true", { stdio: 'inherit' });
    execSync("fi", { stdio: 'inherit' });

    execSync("if [ -f \"$DOCS_DIR/06-deployment.md\" ]; then", { stdio: 'inherit' });
    console.log("-e   ${YELLOW}→ Moving deployment guides${NC}");
    execSync("mv \"$DOCS_DIR/06-deployment.md\" \"$DOCS_DIR/operations/deployment-guide.md\" 2>/dev/null || true", { stdio: 'inherit' });
    execSync("fi", { stdio: 'inherit' });

    execSync("if [ -f \"$DOCS_DIR/07-modules-components.md\" ]; then", { stdio: 'inherit' });
    console.log("-e   ${YELLOW}→ Moving component documentation${NC}");
    execSync("mv \"$DOCS_DIR/07-modules-components.md\" \"$DOCS_DIR/development/components.md\" 2>/dev/null || true", { stdio: 'inherit' });
    execSync("fi", { stdio: 'inherit' });

    execSync("if [ -f \"$DOCS_DIR/08-security-compliance.md\" ]; then", { stdio: 'inherit' });
    console.log("-e   ${YELLOW}→ Moving security documentation${NC}");
    execSync("mv \"$DOCS_DIR/08-security-compliance.md\" \"$DOCS_DIR/enterprise/security-compliance.md\" 2>/dev/null || true", { stdio: 'inherit' });
    execSync("fi", { stdio: 'inherit' });

    execSync("if [ -f \"$DOCS_DIR/09-maintenance.md\" ]; then", { stdio: 'inherit' });
    console.log("-e   ${YELLOW}→ Moving maintenance guides${NC}");
    execSync("mv \"$DOCS_DIR/09-maintenance.md\" \"$DOCS_DIR/operations/maintenance.md\" 2>/dev/null || true", { stdio: 'inherit' });
    execSync("fi", { stdio: 'inherit' });

    execSync("if [ -f \"$DOCS_DIR/10-appendices.md\" ]; then", { stdio: 'inherit' });
    console.log("-e   ${YELLOW}→ Moving appendices${NC}");
    execSync("mv \"$DOCS_DIR/10-appendices.md\" \"$DOCS_DIR/reference/appendices.md\" 2>/dev/null || true", { stdio: 'inherit' });
    execSync("fi", { stdio: 'inherit' });
    ...
    execSync("mv \"$DOCS_DIR/CI-BRANCHING-STRATEGY.md\" \"$DOCS_DIR/development/ci-branching.md\" 2>/dev/null || true", { stdio: 'inherit' });
    execSync("fi", { stdio: 'inherit' });

    execSync("if [ -f \"$DOCS_DIR/COMPLIANCE-ENGINE.md\" ]; then", { stdio: 'inherit' });
    console.log("-e   ${YELLOW}→ Moving compliance documentation${NC}");
    execSync("mv \"$DOCS_DIR/COMPLIANCE-ENGINE.md\" \"$DOCS_DIR/enterprise/compliance-engine.md\" 2>/dev/null || true", { stdio: 'inherit' });
    execSync("fi", { stdio: 'inherit' });

    execSync("if [ -f \"$DOCS_DIR/claude-desktop-integration.md\" ]; then", { stdio: 'inherit' });
    console.log("-e   ${YELLOW}→ Moving integration guides${NC}");
    execSync("mv \"$DOCS_DIR/claude-desktop-integration.md\" \"$DOCS_DIR/examples/claude-integration.md\" 2>/dev/null || true", { stdio: 'inherit' });
    execSync("fi", { stdio: 'inherit' });

    execSync("if [ -f \"$DOCS_DIR/adk-integration.md\" ]; then", { stdio: 'inherit' });
    execSync("mv \"$DOCS_DIR/adk-integration.md\" \"$DOCS_DIR/examples/adk-integration.md\" 2>/dev/null || true", { stdio: 'inherit' });
    execSync("fi", { stdio: 'inherit' });

    execSync("if [ -f \"$DOCS_DIR/voice-agent-compliance.md\" ]; then", { stdio: 'inherit' });
    execSync("mv \"$DOCS_DIR/voice-agent-compliance.md\" \"$DOCS_DIR/specifications/voice-agent.md\" 2>/dev/null || true", { stdio: 'inherit' });
    execSync("fi", { stdio: 'inherit' });
    ...
    execSync("TOTAL_FILES=$(find \"$DOCS_DIR\" -type f -name \"*.md\" -not -path \"$ARCHIVE_DIR/*\" | wc -l)", { stdio: 'inherit' });
