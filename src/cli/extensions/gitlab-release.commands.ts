...
tagCommand
  ...
  .action(async (options: { name: string; ref: string; message?: string; output: string }) => {
    try {
...
tagCommand
  ...
  .option('--output <format>', 'Output format (json|text)', 'text')
  .action(async (options: { type: string; version?: string; page: string; perPage: string; output: string }) => {
    try {
      const config = getGitLabConfig();
      const tagService = new TagService(config.token, config.projectId);
...
milestoneCommand
  ...
  .option('--per-page <count>', 'Items per page', '20')
  .option('--output <format>', 'Output format (json|text)', 'text')
  .action(async (options: { state?: string; page: string; perPage: string; output: string }) => {
    try {
      const config = getGitLabConfig();
      const milestoneService = new MilestoneService(config.token, config.projectId);
...
const incrementDevCommand = new Command('increment-dev')
  ...
  .option('--output <format>', 'Output format (json|text)', 'text')
  .action(async (options: { baseVersion?: string; ref: string; output: string }) => {
    try {
      const config = getGitLabConfig();
      const tagService = new TagService(config.token, config.projectId);
