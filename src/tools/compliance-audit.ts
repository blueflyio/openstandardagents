...
if (!TOKEN) {
  ...
  console.error(
    '   Set environment variable: export GITLAB_TOKEN=<your-token>'
  );
  process.exit(1);
}
...
// Audit functions
async function auditProtectedBranches(
  projectId: string
  ...
  try {
    ...
    const mainProtected = protectedBranches.find(
      (pb: any) => pb.name === 'main' || pb.name === 'master'
    );
    if (mainProtected) {
      checks.push(
        createCheck(
    ...
    const devProtected = protectedBranches.find(
      (pb: any) => pb.name === 'development' || pb.name === 'develop'
    );
    if (devProtected) {
      checks.push(
        createCheck(
...
async function auditMergeRequestApprovals(
  projectId: string
): Promise<ComplianceCheck[]> {
  const checks: ComplianceCheck[] = [];
  ...
  try {
    ...
    try {
      ...
      if (rules && rules.length > 0) {
        ...
        const hasCodeOwners = rules.some(
          (rule: any) => rule.rule_type === 'code_owner'
        );
        if (hasCodeOwners) {
          checks.push(
            createCheck(
...
async function auditSecurityScanning(
  projectId: string
): Promise<ComplianceCheck[]> {
  const checks: ComplianceCheck[] = [];
  ...
  try {
    ...
    const cicdFile = await gitlab.RepositoryFiles.show(
      projectId,
      '.gitlab-ci.yml',
      'HEAD'
    );
    const cicdContent = Buffer.from(cicdFile.content, 'base64').toString(
      'utf-8'
    );
    ...
    if (
      cicdContent.includes('dependency_scanning') ||
      cicdContent.includes('Dependency-Scanning')
    ) {
      checks.push(
        createCheck(
          category,
    ...
    if (
      cicdContent.includes('container_scanning') ||
      cicdContent.includes('Container-Scanning')
    ) {
      checks.push(
        createCheck(
          category,
    ...
    if (
      cicdContent.includes('secret_detection') ||
      cicdContent.includes('Secret-Detection')
    ) {
      checks.push(
        createCheck(
          category,
...
async function auditDORAMetrics(projectId: string): Promise<ComplianceCheck[]> {
  ...
  try {
    ...
    if (
      cicdContent.includes('environment:') ||
      cicdContent.includes('deployment')
    ) {
      checks.push(
        createCheck(
          category,
...
async function auditSecurityPolicies(
  projectId: string
): Promise<ComplianceCheck[]> {
  const checks: ComplianceCheck[] = [];
  ...
  try {
    ...
    try {
      const policyFile = await gitlab.RepositoryFiles.show(
        projectId,
        '.gitlab/security-policies/policy.yml',
        'HEAD'
      );
      const policyContent = Buffer.from(policyFile.content, 'base64').toString(
        'utf-8'
      );
        ...
        if (policyContent.includes('scan: sast')) {
          checks.push(
            createCheck(
              ...
              'SAST Policy',
              'pass',
              'SAST enforcement policy configured'
            )
          );
        }
        if (policyContent.includes('scan: secret_detection')) {
          checks.push(
            createCheck(
              ...
              'pass',
              'Secret detection enforcement policy configured'
            )
          );
        }
        if (policyContent.includes('scan: dependency_scanning')) {
          checks.push(
            createCheck(
              ...
              'pass',
              'Dependency scanning enforcement policy configured'
            )
          );
        }
        if (policyContent.includes('scan: container_scanning')) {
          checks.push(
            createCheck(
              ...
              'pass',
              'Container scanning enforcement policy configured'
            )
          );
        }
      } else {
...
async function auditPipelineConfiguration(
  projectId: string
): Promise<ComplianceCheck[]> {
  const checks: ComplianceCheck[] = [];
  ...
  try {
    ...
    if (pipelines && pipelines.length > 0) {
      ...
      const failCount = pipelines.filter(
...
function displayReport(report: AuditReport): void {
  ...
  console.log(
    `ℹ️  Info: ${report.summary.total - report.summary.passed - report.summary.failed - report.summary.warnings}`
  );
  console.log('='.repeat(80) + '\n');
}
...
function saveReport(report: AuditReport, filename: string): void {
  ...
  const outputFile =
    filename ||
    `compliance-audit-${report.projectId.replace('/', '-')}-${timestamp}.json`;
