...
export class OSSAHoverProvider implements vscode.HoverProvider {
  ...
  provideHover(
    ...
    const fieldDocs: Record<string, string> = {
      ...
      metadata:
        'Metadata section containing name, version, description, and tags.',
      ...
      provider:
        'LLM provider: anthropic, openai, azure, bedrock, gemini, mistral, ollama.',
      model: 'Specific model identifier (e.g., claude-sonnet-4-20250514).',
      tools:
        'List of tools available to the agent (capabilities or functions).',
