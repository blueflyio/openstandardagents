...
function main() {
  ...
  baseSchema.$id =
    'https://openstandardagents.org/schemas/v0.3.5/manifest.json';
  baseSchema.title = 'OSSA v0.3.5 Manifest Schema';
  baseSchema.description =
    'Open Standard for Software Agents (OSSA) v0.3.5 - The Next OpenAPI for Software Agents. A specification standard (not a framework) that defines contracts/metadata for production agent systems. Includes completion signals, checkpointing, MoE, BAT framework, MOE metrics, Flow kind, capability discovery, feedback loops, and infrastructure substrate.';

  // Update apiVersion pattern to include v0.3.5
  baseSchema.properties.apiVersion.pattern =
    '^ossa/v(0\\.3\\.[4-9]|0\\.3\\.[0-9]+(-[a-zA-Z0-9]+)?|0\\.2\\.[2-9](-dev)?|1)(\\.[0-9]+)?(-[a-zA-Z0-9]+)?$';
  baseSchema.properties.apiVersion.examples = [
    'ossa/v0.3.5',
    'ossa/v0.3.4',
    'ossa/v1',
  ];
  ...
  for (const file of extensionFiles) {
    ...
    try {
      ...
      console.error(
        `  ❌ Failed to merge ${file}: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
