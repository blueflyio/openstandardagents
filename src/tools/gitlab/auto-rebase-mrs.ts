...
async function rebaseMR(mr: any): Promise<boolean> {
  ...
  try {
    // Fetch branches
    execSync(`git fetch origin ${source_branch} ${target_branch}`, {
      stdio: 'inherit',
    });
    ...
    execSync(`git worktree add ${worktreePath} origin/${source_branch}`, {
      stdio: 'inherit',
    });

    // Rebase
    try {
      execSync(`cd ${worktreePath} && git rebase origin/${target_branch}`, {
        stdio: 'inherit',
      });

      // Push
      execSync(
        `cd ${worktreePath} && git push origin ${source_branch} --force-with-lease`,
        { stdio: 'inherit' }
      );
