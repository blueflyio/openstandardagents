...
export function createEnvCleanupCommand(): Command {
  ...
  command
    .description(
      'Manage GitLab environments - list and stop stuck environments'
    )
    ...
    .action((options) => {
      try {
        ...
        if (options.stopStuck) {
          ...
          console.log(
            `Found ${stuck.length} stuck environments. Stopping them...`
          );
          stuck.forEach((env) => {
          ...
          console.log(
            'They may remain in "stopping" state. Manual deletion via GitLab UI may be needed:'
          );
          console.log(
            'https://gitlab.com/blueflyio/ossa/openstandardagents/-/environments'
          );
          return;
        }
