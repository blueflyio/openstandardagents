    ...
    try {
      ...
      if (spec.openapi) {
        ...
        if (!spec.info) {
          ...
          if (!spec.info.title)
            errors.push('OpenAPI spec missing "info.title"');
          if (!spec.info.version)
            errors.push('OpenAPI spec missing "info.version"');
        }
        ...
        errors.push(
          'Spec missing $schema, openapi, or recognized spec format identifier'
        );
      }
