...
export class SpecificationValidator {
   ...
   */
  private validateAgentManifestSchema(manifest: any): {
    valid: boolean;
   ...
   */
  private validateOSSACompliance(manifest: any): {
    errors: Array<{ field: string; message: string; code: string }>;
    warnings: string[];
  } {
    const errors: Array<{ field: string; message: string; code: string }> = [];
    ...
    if (manifest.spec?.capabilities?.domains) {
      const invalidDomains = manifest.spec.capabilities.domains.filter(
        (domain: string) => !this.validDomains.includes(domain)
      );
   ...
   */
  private validateProtocols(protocols: any): {
    errors: Array<{ field: string; message: string; code: string }>;
    warnings: string[];
  } {
    const errors: Array<{ field: string; message: string; code: string }> = [];
   ...
   */
  private validateConformance(conformance: any): {
    errors: Array<{ field: string; message: string; code: string }>;
    warnings: string[];
  } {
    const errors: Array<{ field: string; message: string; code: string }> = [];
   ...
   */
  private validateBudgets(budgets: any): {
    errors: Array<{ field: string; message: string; code: string }>;
    warnings: string[];
  } {
    const errors: Array<{ field: string; message: string; code: string }> = [];
    ...
    return {
      ...
      types: [
        ...
        },
        {
          name: 'critic',
          description:
            'Provides multi-dimensional reviews and feedback across quality, security, and performance dimensions',
          capabilities: [
            'review-generation',
            'feedback-analysis',
            'quality-assessment',
