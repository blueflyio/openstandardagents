...
program
  ...
  .action(async (version: string, options) => {
    ...
    try {
      ...
      console.log(`  Replacements made: ${chalk.bold(result.replacementsMade)}`);

      if (result.files.length > 0) {
        console.log(chalk.cyan('\nFiles modified:'));
        result.files.forEach((file) => {
          console.log(`  ${chalk.gray(file.path)} (${file.replacements} replacements)`);
        });
      }
...
program
  ...
  .action(async (version: string | undefined, options) => {
    if (!version && !options.all) {
      console.error(chalk.red('Error: Either provide a version or use --all flag'));
      process.exit(1);
    }
    ...
    try {
      ...
      if (result.versionRestored) {
        console.log(`  Version restored: ${chalk.bold(result.versionRestored)}`);
      }
      console.log(`  Files processed: ${chalk.bold(result.filesProcessed)}`);
      console.log(`  Replacements made: ${chalk.bold(result.replacementsMade)}`);
...
program
  ...
  .option(
    ...
    ['git_tag', 'package_json', 'composer_json', 'VERSION_file', 'pyproject_toml', 'cargo_toml']
  )
  .action(async (options) => {
    ...
    try {
      ...
      spinner.succeed(chalk.green(`Detected version: ${chalk.bold(result.version)}`));
      ...
      if (Object.keys(result.allSources).length > 1) {
        ...
        Object.entries(result.allSources).forEach(([source, version]) => {
          const indicator = source === result.source ? chalk.green('✓') : chalk.gray(' ');
...
program
  ...
  .action((version: string) => {
    ...
    if (result.valid) {
      console.log(chalk.green(`✓ Valid semantic version: ${chalk.bold(version)}`));
      ...
      console.log(chalk.red(`✗ Invalid semantic version: ${chalk.bold(version)}`));
...
program
  ...
  .option('--prerelease <identifier>', 'Prerelease identifier (e.g., alpha, beta, rc)')
  .action((version: string, type: string, options) => {
    try {
      ...
      if (!['major', 'minor', 'patch', 'prerelease'].includes(bumpType)) {
        throw new Error('Bump type must be: major, minor, patch, or prerelease');
      }
      ...
      console.log(`  New version: ${chalk.bold(chalk.green(result.newVersion))}`);
