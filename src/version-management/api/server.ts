...
app.post('/api/v1/version/substitute', async (req: Request, res: Response, next: NextFunction) => {
  try {
...
app.post('/api/v1/version/restore', async (req: Request, res: Response, next: NextFunction) => {
  try {
...
app.get('/api/v1/version/detect', async (req: Request, res: Response, next: NextFunction) => {
  try {
...
app.post('/api/v1/version/bump', (req: Request, res: Response, next: NextFunction) => {
  try {
    const { version, bump_type, prerelease_identifier } = req.body;
    const result = versionManager.bump(version, bump_type, prerelease_identifier);
    res.json(result);
  } catch (error) {
    next(error);
  }
